...
import type { ArcChatMessagePayload, WakePayload } from "./types";
    ...
    if (method === "POST" && url === config.webhookPath) {
      ...
      if (config.webhookSecret) {
        ...
        if (!verifySignature(rawBody, signature, config.webhookSecret)) {
          console.warn("[arc-runner] wake REJECTED: bad/missing signature (app's ARC_WEBHOOK_SECRET must match the bridge's)");
      ...
      if (payload.type === "arc_chat_message") {
        ...
        void handleChatMessage(client, config, payload as ArcChatMessagePayload);
        return;
      }
