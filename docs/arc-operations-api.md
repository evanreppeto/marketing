# Arc Operations API (`/api/v1/arc/*`)

Secure, structured app access for the Arc agent so it operates from
stable APIs instead of browser scraping. Every route is bearer-gated with
`ARC_AGENT_API_TOKEN` and (except `/health`) requires Supabase admin env
...
Arc can **read, advise, log, and drive its own task lifecycle** — it can never
...
- No agent route imports `@/lib/approvals/decisions`, `@/lib/campaigns/decisions`,
  ...
  `src/app/api/v1/arc/__tests__/safety.test.ts`.
...
| Method | Path | Notes |
| --- | --- | --- |
| GET | `/health` | `{ service: "bsr-marketing-arc", supabaseConfigured }`. Bearer-only. |
...
| GET | `/approvals/:id` | Single approval card (any status) **+ Arc's recommendations**. |
...
```jsonc
{
  ...
  "assignee",              // "arc" | agent name | "unassigned"
...
Built: **`arc-runner/mcp_server.py`** — a Python MCP server (stdio) wrapping
these routes as tools, reading `BSR_MARKETING_BASE_URL` (falls back to
`APP_BASE_URL`) + `ARC_AGENT_API_TOKEN` and sending the bearer header on every
call via stdlib `urllib`. Run: `pip install -r arc-runner/requirements.txt` then
`python arc-runner/mcp_server.py`. Tools: `health`, `list_tasks`, `get_task`,
...
## TODO — `campaigns/:id/arc-note` (deferred)
