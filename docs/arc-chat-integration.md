# Arc ⇄ Arc Chat — Agent Integration Contract

This is everything your external **Arc agent** needs to power the in-app
chat at `/arc`. The app holds the conversation; your agent supplies the replies.
Everything is plain HTTP with one bearer token — the same pattern as
`/api/v1/arc/runs`. **Outbound stays locked**: the chat never sends anything
...
`ARC_AGENT_API_TOKEN` must be set in the app's deployment env (Vercel). Same
token gates `/runs`, `/ping`, and the chat endpoints below.

Base URL = your deployment, e.g. `https://<your-vercel-app>` (use `/api/v1/arc/ping` to confirm reachability + auth).

## The loop

```
   ┌────────────────────────────────────────────────────────────┐
   │ 1. operator types in /arc  → app queues an agent_task       │
   │ 2. your agent: GET /api/v1/arc/messages   (pull queue)    │
   │ 3. your agent does the work (read data, run /runs, reason…)  │
   │ 4. your agent: POST /api/v1/arc/messages  (deliver reply) │
...
Instead of polling, let the app wake Arc only when a message is sent. Set two
env vars on **Arc's** side and one on the app:

- App (Vercel): `ARC_WEBHOOK_URL` = the agent endpoint to call; optional
  `ARC_WEBHOOK_SECRET` = a shared secret.

When an operator sends a chat message, the app immediately `POST`s to
`ARC_WEBHOOK_URL`:

```
POST <ARC_WEBHOOK_URL>
Authorization: Bearer <ARC_WEBHOOK_SECRET>   (only if you set the secret)
...
2. Compose the answer, then deliver it with the reply call in section 2:
   `POST /api/v1/arc/messages { agentTaskId, body, metadata? }`.

That's it — no polling, no idle cost; Arc runs only when there's a message.
...
### Attaching media (images / video Arc created)
...
`task_type = 'arc_chat_message'`, `status = 'queued'`, `objective = <message>`,
and `metadata = { conversation_id, message_id, mentions, requested_by, outbound_locked: true }`.
The conversation + messages live in `public.arc_conversations` / `public.arc_messages`.
Prefer the HTTP endpoints above — they keep the "thinking" bubble and task status
in sync for you.

## Related endpoints (already live)

- `GET  /api/v1/arc/ping` — connectivity + auth + `supabaseConfigured` check.
- `POST /api/v1/arc/runs` — run the deterministic partner-campaign workflow
