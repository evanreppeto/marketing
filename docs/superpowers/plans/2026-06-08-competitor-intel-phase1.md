...
- Create `src/app/api/v1/arc/competitor-intel/route.ts` — thin wrapper (no test; matches house convention).
- Create `C:\Users\evanr\marketing-classifier-agent\arc-skills\competitor-intel-scout\SKILL.md`.
...
**Files:** Create `src/app/api/v1/arc/competitor-intel/route.ts`.
...
- [ ] **Step 3: Commit** — `git add "src/app/api/v1/arc/competitor-intel/route.ts" && git commit -m "feat(api): add POST /api/v1/arc/competitor-intel"`

## Task 5: Arc skill — `competitor-intel-scout`

**Files:** Create `C:\Users\evanr\marketing-classifier-agent\arc-skills\competitor-intel-scout\SKILL.md`.

- [ ] **Step 1: Write the skill** — tells Arc which sources to browse (Meta Ad Library, Google Ads Transparency, SimilarWeb, competitor landing pages), exactly which fields to extract (source, competitorName, competitorUrl, summary, channelMix, estSpend, topKeywords[], adCreatives[{headline,body,mediaUrl}]), the **POST `{APP_URL}/api/v1/arc/competitor-intel`** shape with bearer `ARC_AGENT_API_TOKEN`, success `201 {result:{competitorCampaignId, status:"needs_review"}}`, and a ToS caution (respect robots/rate limits; everything lands `needs_review`, never auto-trusted, never triggers outbound). (Deliverable for Arc's Mac profile; not committed to the app repo.)
...
## Self-Review
- **Spec coverage:** structured competitor records (Task 1–3) ✅; bearer-gated endpoint mirroring runs (Task 4) ✅; Arc procedure (Task 5) ✅; review path (Task 6, interim) ✅. Dropped vs old design: `vault_notes` note + operator UI (Phase 2) — intentional, logged.
...
- **Assumption to verify at execution:** `OFFICIAL_PERSONA_MAPPINGS` is exported from `src/domain/personas` (used by existing `arc/contracts.ts` via `@/domain`); confirm the import path resolves (use `@/domain` if the direct `./personas` path differs).
