# Mark Kanban Board Implementation Plan (Plan 1 of 2)
...
**Architecture:** Pure transition logic lives in `src/domain/hermes-tasks.ts` (unit-tested). Persistence (`moveAgentTask`) extends the existing `src/lib/hermes-api/tasks.ts` lifecycle layer. Operator-facing mutations are `requireOperator()`-gated server actions in `src/app/agent-operations/actions.ts`. The board itself is a client component that calls those actions and refreshes. No core schema change — the board is backed by the existing `agent_tasks` table.
...
**Spec:** `docs/superpowers/specs/2026-06-10-mark-kanban-board-design.md`
...
|------|----------------|--------|
| `src/domain/hermes-tasks.ts` | Add `canOperatorMoveTask` + drag transition map | Modify |
| `src/domain/__tests__/hermes-tasks.test.ts` | Tests for the above | Modify |
| `src/lib/hermes-api/tasks.ts` | Add `moveAgentTask` persistence | Modify |
| `src/lib/hermes-api/__tests__/tasks.test.ts` | Tests for `moveAgentTask` | Modify |
...
**Files:**
- Modify: `src/domain/hermes-tasks.ts`
- Test: `src/domain/__tests__/hermes-tasks.test.ts`
...
Add to the bottom of `src/domain/__tests__/hermes-tasks.test.ts`:

```ts
import { canOperatorMoveTask } from "../hermes-tasks";
...
Run: `pnpm test src/domain/__tests__/hermes-tasks.test.ts`
Expected: FAIL — `canOperatorMoveTask is not a function`.

- [ ] **Step 3: Write minimal implementation**

Append to `src/domain/hermes-tasks.ts`:
...
```bash
git add src/domain/hermes-tasks.ts src/domain/__tests__/hermes-tasks.test.ts
git commit -m "feat(mark): operator drag-transition rules for the task board"
```
...
**Files:**
- Modify: `src/lib/hermes-api/tasks.ts`
- Test: `src/lib/hermes-api/__tests__/tasks.test.ts`
...
Open `src/lib/hermes-api/__tests__/tasks.test.ts`, look at the top to reuse its existing Supabase mock helper style, then add this block (it uses the same fake-client shape the file already uses for `claimAgentTask`; if the file builds a fresh stub per test, follow that local pattern instead):
          ...
          update: (patch: Record<string, unknown>) => {
            ...
            return {
              eq: () => ({
                select: () => ({
                  single: async () => ({
                    data: { ...(opts.taskRow ?? {}), ...patch, agents: { key: "mark", name: "Mark" } },
                    error: null,
                  }),
                }),
              }),
...
Run: `pnpm test src/lib/hermes-api/__tests__/tasks.test.ts`
Expected: FAIL — `moveAgentTask is not exported`.

- [ ] **Step 3: Write minimal implementation**

Add to `src/lib/hermes-api/tasks.ts`. Import `canOperatorMoveTask` and `type OperatorDropTarget` from `@/domain` (add them to the existing import from `@/domain` at the top). Then:
...
```bash
git add src/lib/hermes-api/tasks.ts src/lib/hermes-api/__tests__/tasks.test.ts
git commit -m "feat(mark): moveAgentTask persistence with approval guardrails"
```
...
(If `moveAgentTask` is not yet re-exported from `@/lib/hermes-api`, add `export * from "./tasks";`-style re-export — check `src/lib/hermes-api/index.ts` and add `moveAgentTask` to its exports if it lists names explicitly.)
...
```bash
git add src/app/agent-operations/actions.ts src/lib/hermes-api/index.ts
git commit -m "feat(mark): moveTaskAction operator-gated board move"
```
...
git commit -m "feat(mark): free-form createTaskAction for the board"
```
...
git commit -m "feat(mark): Kanban board component with swimlanes and drag"
```
...
git commit -m "feat(mark): new-task dialog and board/table view switch"
```
...
- The board renders with a Mark swimlane and 5 columns.
...
git commit -m "feat(mark): mount Kanban board on the agent-operations page"
```
...
- **Productization (§9):** no new hardcoded "Mark" in the rendering path (lanes derive from the `agents` table / task rows; `ensureMarkAgent` stays the isolated seed). Domain rule `canOperatorMoveTask` is tenant-agnostic. UI filters tasks by `agentKey` (never assumes a global list).
...
- **Integration points (verified during authoring):** `@/domain/index.ts` already does `export * from "./hermes-tasks"`, so the new symbols auto-export. `@/lib/hermes-api/index.ts` already does `export * from "./tasks"`, so `moveAgentTask` auto-exports (no edit to `index.ts` needed in Task 3 — drop that `git add` of `index.ts`). `ButtonVariant` includes `"primary"` and `"ghost"`; `ThemeTone` = `amber | green | red | blue | gray | dark` — all tones used in Tasks 5–6 are valid.
```
