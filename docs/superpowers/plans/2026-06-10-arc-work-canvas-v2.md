...
**Goal:** Make the Arc Work Canvas live and editable — the operator types directly into an agent draft, sees it in a channel-true frame (Gmail / Meta / SMS / generic), and saves edits that persist to the backing `campaign_assets` row. Outbound stays locked.

**Architecture:** Pure channel classification + field resolution in `src/domain/arc-canvas.ts`; a read/write layer in `src/lib/campaigns/draft-editing.ts`; two operator-gated server actions; and client UI (`editable-field.tsx` primitive → `channel-preview.tsx` frames → `channel-artifact.tsx` live container) wired into the existing `work-canvas.tsx`. Body edits go to `campaign_assets.edited_body`; structured fields (subject/headline/CTA) go to a new `edited_fields jsonb` column, leaving Arc's `draft_body`/`prompt_inputs` pristine.
...
- Create: `src/domain/arc-canvas.ts` — pure channel/field logic.
- Modify: `src/domain/index.ts` — re-export the new module.
- Create: `src/domain/__tests__/arc-canvas.test.ts` — domain unit tests.
- Create: `src/lib/campaigns/draft-editing.ts` — `getDraftAsset` + `editDraftAsset`.
- Modify: `src/app/arc/actions.ts` — `getDraftAssetAction` + `editDraftAssetAction`.
- Create: `src/app/arc/_components/editable-field.tsx` — inline-edit primitive.
- Create: `src/app/arc/_components/channel-preview.tsx` — Email/Meta/SMS/Generic frames.
- Create: `src/app/arc/_components/channel-artifact.tsx` — live fetch + save container.
- Modify: `src/app/arc/_components/work-canvas.tsx` — route draft cards with a backing asset to `ChannelArtifact`.
...
git commit -m "feat(arc): add campaign_assets.edited_fields + asset_edited event"
```

---

## Task 2: Domain — `arc-canvas.ts` channel classification + field resolution (TDD)

**Files:**
- Create: `src/domain/arc-canvas.ts`
- Test: `src/domain/__tests__/arc-canvas.test.ts`
- Modify: `src/domain/index.ts`

- [ ] **Step 1: Write the failing tests**

Create `src/domain/__tests__/arc-canvas.test.ts`:
...
Run: `pnpm test src/domain/__tests__/arc-canvas.test.ts`
Expected: FAIL — `Cannot find module '../arc-canvas'`.

- [ ] **Step 3: Implement `src/domain/arc-canvas.ts`**
...
In `src/domain/index.ts`, add after the existing `export * from "./arc-chat";` line:
...
```bash
git add src/domain/arc-canvas.ts src/domain/__tests__/arc-canvas.test.ts src/domain/index.ts
git commit -m "feat(arc): pure channel-preview classification + draft field resolution"
```
...
git commit -m "feat(arc): draft-editing read/write layer (getDraftAsset + editDraftAsset)"
```
...
At the top of `src/app/arc/actions.ts`, add to the existing import block from the campaigns libs (next to the `decideAsset` import on the line `import { type ApprovalDecision, decideAsset } from "@/lib/campaigns/decisions";`):
...
git commit -m "feat(arc): getDraftAssetAction + editDraftAssetAction (operator-gated)"
```
...
**Files:**
- Create: `src/app/arc/_components/editable-field.tsx`
...
```bash
git add src/app/arc/_components/editable-field.tsx
git commit -m "feat(arc): inline EditableField primitive for the work canvas"
```
...
**Files:**
- Create: `src/app/arc/_components/channel-preview.tsx`
...
```bash
git add src/app/arc/_components/channel-preview.tsx
git commit -m "feat(arc): channel-true preview frames (email/meta/sms/generic)"
```
...
**Files:**
- Create: `src/app/arc/_components/channel-artifact.tsx`
  ...
  if (loading && !view) {
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="arc-skel" style={{ width: "88%" }} />
      </div>
    );
  }
...
```bash
git add src/app/arc/_components/channel-artifact.tsx
git commit -m "feat(arc): live ChannelArtifact container (fetch, edit, save, approve)"
```
...
**Files:**
- Modify: `src/app/arc/_components/work-canvas.tsx`
...
git commit -m "feat(arc): route editable draft cards to the live channel artifact"
```
...
Run: `pnpm test`
Expected: PASS, including the new `arc-canvas.test.ts`.
