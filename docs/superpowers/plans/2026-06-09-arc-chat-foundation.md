# Arc Chat — Foundation Pass Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix the missing-projects-schema bug that breaks the Arc chat sidebar, then make every control work premium-grade (search, pin, rename, delete, copy, retry, stop, richer thinking state).

**Architecture:** One additive migration creates `arc_projects` + adds `project_id`/`pinned_at` to `arc_conversations`. New persistence helpers + server actions back pin/delete/cancel. The chat UI gains a shared `IconButton`, an extracted `useThreadPoll` hook, a `ThreadMenu` overflow popover, sidebar search, header inline-rename, a message hover toolbar, and a richer pending-reply state. Uploads/slash-commands/connections are deferred but get reserved UI slots.
...
**Spec:** `docs/superpowers/specs/2026-06-09-arc-chat-foundation-design.md`

---

## File map

- Create `supabase/migrations/20260609130000_arc_projects_and_pins.sql` — `arc_projects` table + `project_id`/`pinned_at` columns + indexes.
- Modify `src/lib/arc-chat/persistence.ts` — `pinnedAt` on `MarkConversation`/`ConversationRow`/`CONVERSATION_COLUMNS`/`toConversation`; pinned ordering in `listConversations`/`listArchivedConversations`; new `setConversationPinned`, `deleteConversation`, `cancelPendingMarkMessage`.
- Create `src/lib/arc-chat/persistence.test.ts` — tests for the new helpers + ordering.
- Modify `src/app/arc/actions.ts` — `renameThreadForm`, `pinThreadForm`, `unpinThreadForm`, `deleteThreadForm`, `renameProjectForm`, `cancelReplyAction`.
- Create `src/app/arc/_components/icon-button.tsx` — shared small icon button.
- Create `src/app/arc/_components/relative-time.ts` + `relative-time.test.ts` — pure `relativeTime(iso, nowMs)`.
- Create `src/app/arc/_components/use-thread-poll.ts` + `use-thread-poll.test.ts` — extracted polling hook + `sameMessages`.
- Create `src/app/arc/_components/thread-menu.tsx` — per-row `⋯` overflow popover.
- Modify `src/app/arc/_components/thread-sidebar.tsx` — search box, sections (Pinned/Project/Chats), relative timestamps, `ThreadMenu`.
- Modify `src/app/arc/_components/arc-chat.tsx` — header inline-rename + meta + `⋯` + connections slot; use `useThreadPoll`; pass retry/cancel callbacks.
- Modify `src/app/arc/_components/message-list.tsx` — hover toolbar (Copy/Retry), richer thinking state (breathe/shimmer/timeline line/elapsed/Stop), References cluster.
- Modify `src/app/arc/_components/composer.tsx` — expose programmatic send for Retry; reserve leading `+` slot.
- Modify `src/app/arc/_components/empty-state.tsx` — marketing-intent suggestion cards.
- Modify `src/app/globals.css` — `avatar-breathe` + `text-shimmer` keyframes + reduced-motion entries.

---

## Task 1: Migration — arc_projects + project_id + pinned_at

**Files:**
- Create: `supabase/migrations/20260609130000_arc_projects_and_pins.sql`
...
-- arc_projects and arc_conversations.project_id, but no migration created them,
...
create index arc_projects_operator_idx on public.arc_projects(operator, created_at);
...
alter table public.arc_conversations
  add column project_id uuid references public.arc_projects(id) on delete set null;

-- Pin a conversation to the top of the list.
alter table public.arc_conversations
  add column pinned_at timestamptz;

create index arc_conversations_pin_idx
...
```bash
git add supabase/migrations/20260609130000_arc_projects_and_pins.sql
git commit -m "feat(arc-chat): add arc_projects + project_id/pinned_at migration"
```
...
- Test: `src/lib/arc-chat/persistence.test.ts` (create)

- [ ] **Step 1: Write the failing test**

Create `src/lib/arc-chat/persistence.test.ts`:
...
Run: `pnpm vitest run src/lib/arc-chat && pnpm lint`
Expected: PASS.
...
git commit -m "feat(arc-chat): pinnedAt + pin/delete/cancel-pending persistence helpers"
```
...
In `src/app/arc/actions.ts`, add these names to the existing `from "@/lib/arc-chat/persistence"` import block:
```typescript
  cancelPendingMarkMessage,
  deleteConversation,
  renameProject,
...
Append to `src/app/arc/actions.ts`:
...
git commit -m "feat(arc-chat): pin/unpin/delete/rename-form + cancelReply server actions"
```
...
**Files:**
- Create: `src/app/arc/_components/icon-button.tsx`
...
```bash
git add src/app/arc/_components/icon-button.tsx
git commit -m "feat(arc-chat): shared IconButton primitive"
```
...
**Files:**
- Create: `src/app/arc/_components/relative-time.ts`
- Test: `src/app/arc/_components/relative-time.test.ts`
...
Run: `pnpm vitest run src/app/arc/_components/relative-time.test.ts`
...
```bash
git add src/app/arc/_components/relative-time.ts src/app/arc/_components/relative-time.test.ts
git commit -m "feat(arc-chat): relativeTime helper for thread row timestamps"
```
...
**Files:**
- Create: `src/app/arc/_components/use-thread-poll.ts`
- Test: `src/app/arc/_components/use-thread-poll.test.ts`
- Modify: `src/app/arc/_components/arc-chat.tsx` (wire the hook — completed here, header work lands in Task 8)
...
Run: `pnpm vitest run src/app/arc/_components/use-thread-poll.test.ts`
Expected: FAIL — module not found.
...
- [ ] **Step 5: Use the hook in `arc-chat.tsx`, remove the inline polling**
...
```bash
git add src/app/arc/_components/use-thread-poll.ts src/app/arc/_components/use-thread-poll.test.ts src/app/arc/_components/arc-chat.tsx
git commit -m "refactor(arc-chat): extract useThreadPoll hook + tested sameMessages"
```
...
**Files:**
- Create: `src/app/arc/_components/thread-menu.tsx`
...
```bash
git add src/app/arc/_components/thread-menu.tsx
git commit -m "feat(arc-chat): ThreadMenu overflow popover (pin/move/archive/delete)"
```
...
Replace the entire contents of `src/app/arc/_components/thread-sidebar.tsx` with:
...
import { cx } from "@/app/_components/theme";
import type { MarkConversation, MarkProject } from "@/lib/arc-chat/persistence";
...
function NewChatLink() {
  return (
    <Link
      href="/arc"
      aria-label="Start a new chat with Arc"
  ...
  if (showArchived) {
    return (
      <aside className="hidden min-h-0 flex-col gap-2 overflow-y-auto p-3 lg:flex">
        <NewChatLink />
        <Link href="/arc" className="signal-eyebrow px-2 pt-2 hover:text-[var(--text-primary)]">
          ‹ Back to chats
        </Link>
      ...
      <nav aria-label="Conversations" className="flex min-h-0 flex-col gap-0.5">
        {unprojected.length === 0 ? (
          <p className="px-2 py-3 text-xs text-[var(--text-muted)]">
            {q ? "No matches." : "No conversations yet. Say hello to Arc."}
          </p>
        ) : (
          unprojected.map((c) => (
            <ChatRow key={c.id} c={c} projects={projects} activeId={activeId} nowMs={nowMs} />
...
```bash
git add src/app/arc/_components/thread-sidebar.tsx
git commit -m "feat(arc-chat): sidebar search, Pinned section, row timestamps + ThreadMenu"
```
...
```tsx
      <header className="flex items-center justify-between gap-3 pb-3">
        <div className="min-w-0">
          <p className="signal-eyebrow">Arc</p>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {/* Reserved slot: future "what Arc can reach" connections indicator. */}
          {activeId ? (
            <ThreadMenu
...
In `src/app/arc/page.tsx`, update the `<MarkChat .../>` props to include:
```tsx
...
```bash
git add src/app/arc/_components/arc-chat.tsx src/app/arc/page.tsx
git commit -m "feat(arc-chat): header inline-rename, meta line, thread menu + connections slot"
```
...
In `src/app/arc/_components/composer.tsx`:
...
git commit -m "feat(arc-chat): composer exposes programmatic send + reserved attach slot"
```
...
- Modify: `src/app/arc/_components/arc-chat.tsx` (pass `onRetry` + `onStop` down)

- [ ] **Step 1: Add `onRetry`/`onStop` plumbing in `arc-chat.tsx`**
...
Rewrite the Arc/system branch of `Message` to use `PendingBlock`, the hover toolbar (Copy on complete, Retry on failed), and `References` (replacing the loose `MentionChips` for Arc replies):

```tsx
  // Arc / system: full-width, avatar + plain text.
...
git commit -m "feat(arc-chat): message toolbar (copy/retry), richer thinking state, References cluster"
```
...
In `src/app/globals.css`, just after the `.lightbox-panel { ... }` line (around line 353, end of the "Arc chat motion" block), add:
...
In the `@media (prefers-reduced-motion: reduce)` block (around line 430), add `.arc-shimmer` to the list of selectors set to `animation: none;`, and give it a solid color so the text stays visible:
...
Run: `pnpm lint`
Expected: PASS. Manually confirm the dev server compiles CSS without errors (`pnpm dev`, load `/arc`).

- [ ] **Step 4: Commit**

```bash
git add src/app/globals.css
git commit -m "feat(arc-chat): avatar-breathe + text-shimmer keyframes (reduced-motion safe)"
```
...
const SUGGESTIONS = [
  {
    ...
    hint: "Arc searches and proposes who to add",
  ...
  },
  {
    ...
    hint: "Arc drafts; outbound stays locked",
...
git commit -m "feat(arc-chat): marketing-intent empty-state suggestions"
```
...
- [ ] Delete from the menu → inline "Delete? Click to confirm" → row gone; if it was the active thread, you land on `/arc`.
- [ ] Send a message: pending bubble shows the breathing avatar + "Arc is thinking…" shimmer; with steps, the timeline shows the connective line + elapsed timer + Stop.
- [ ] Stop removes the pending bubble and settles the thread.
- [ ] Copy on a Arc reply shows "Copied" for ~1.5s.
...
- **Ordering note:** Task 6 must land before Task 9 (header) and Task 11 (which both edit `arc-chat.tsx`) so the polling extraction doesn't conflict; Tasks are already in that order.
```
