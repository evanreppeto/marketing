# "Talk to Mark" Chat — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A multi-thread, ChatGPT-style page at `/mark` where the operator talks to Mark (the Hermes agent), @-mentions records, sees a "thinking" animation, and gets replies delivered asynchronously via a bearer-gated callback.

**Architecture:** Durable-first, matching the repo. Operator messages persist in two new Supabase tables and enqueue an `agent_task` (extending the existing `sendMarkDirective` pattern); Hermes posts replies back to `POST /api/v1/hermes/messages`, which flips a `pending` Mark message to `complete`. The chat UI polls the active thread while a reply is pending. Outbound stays locked; this app holds no LLM key.
...
| File | Responsibility |
| --- | --- |
| `supabase/migrations/20260608120000_mark_chat.sql` | `mark_conversations` + `mark_messages` tables, indexes, RLS, updated_at triggers. |
| `src/domain/mark-chat.ts` | Pure logic: mention types, `deriveThreadTitle`, `validateMarkMessageInput`, mention serialize/parse. |
| `src/domain/__tests__/mark-chat.test.ts` | Vitest unit tests for the above. |
| `src/domain/index.ts` | Re-export `./mark-chat`. |
| `src/lib/mark-chat/persistence.ts` | Conversation/message repos over the two tables (untyped client). |
| `src/lib/mark-chat/enqueue.ts` | Create the `agent_task` + `agent_task_inputs` for a chat message. |
| `src/lib/mark-chat/mention-search.ts` | `getMentionables()` — one catalog of @-mentionable records from read-models. |
| `src/app/api/v1/hermes/messages/route.ts` | Bearer-gated callback: Hermes posts Mark's reply. |
| `src/app/mark/actions.ts` | `"use server"` actions: send, rename, archive, poll. |
| `src/app/mark/page.tsx` | Server component: loads threads + active messages + mention catalog. |
| `src/app/mark/_components/mark-chat.tsx` | Client shell: state, polling, composer wiring. |
| `src/app/mark/_components/thread-sidebar.tsx` | Thread list + New chat + rename/archive. |
| `src/app/mark/_components/message-list.tsx` | Message rows + thinking indicator + reference cards. |
| `src/app/mark/_components/composer.tsx` | Textarea + @-mention popover + chips. |
| `src/app/_data/growth-engine.ts` | Repoint "Mark" nav to `/mark`. |
...
### Task 1: Migration — `mark_conversations` + `mark_messages`
...
git commit -m "feat(mark-chat): migration for mark_conversations + mark_messages"
```

---

### Task 2: Domain logic — `src/domain/mark-chat.ts` (+ tests)

**Files:**
- Create: `src/domain/mark-chat.ts`
- Create: `src/domain/__tests__/mark-chat.test.ts`
...
Run: `pnpm test src/domain/__tests__/mark-chat.test.ts`
Expected: FAIL (module `../mark-chat` not found).

- [ ] **Step 3: Implement `src/domain/mark-chat.ts`**
...
```bash
git add src/domain/mark-chat.ts src/domain/__tests__/mark-chat.test.ts src/domain/index.ts
git commit -m "feat(mark-chat): domain logic for mentions, titles, message validation"
```

---

### Task 3: Persistence — `src/lib/mark-chat/persistence.ts`

**Files:**
- Create: `src/lib/mark-chat/persistence.ts`

- [ ] **Step 1: Write the persistence module**

Note: `client` is the UNTYPED `SupabaseClient` (so the not-in-Database tables compile). Mirrors `mark-conversation.ts`.
...
```bash
git add src/lib/mark-chat/persistence.ts
git commit -m "feat(mark-chat): conversation + message persistence"
```

---

### Task 4: Enqueue — `src/lib/mark-chat/enqueue.ts`

**Files:**
- Create: `src/lib/mark-chat/enqueue.ts`
...
```bash
git add src/lib/mark-chat/enqueue.ts
git commit -m "feat(mark-chat): enqueue operator messages as agent_tasks"
```

---

### Task 5: Mention catalog — `src/lib/mark-chat/mention-search.ts`

**Files:**
- Create: `src/lib/mark-chat/mention-search.ts`
...
```bash
git add src/lib/mark-chat/mention-search.ts
git commit -m "feat(mark-chat): @-mention catalog from read-models"
```
...
### Task 6: Callback API — `POST /api/v1/hermes/messages`

**Files:**
- Create: `src/app/api/v1/hermes/messages/route.ts`
...
Run: `pnpm build && pnpm lint`
Expected: PASS. Build output should list `/api/v1/hermes/messages` as a route.

- [ ] **Step 3: Commit**

```bash
git add src/app/api/v1/hermes/messages/route.ts
git commit -m "feat(mark-chat): bearer-gated callback for Mark replies"
```

---

### Task 7: Server actions — `src/app/mark/actions.ts`

**Files:**
- Create: `src/app/mark/actions.ts`
...
Note: `enqueueMarkChatTask` is called with `messageId: ""` because the operator message id isn't needed by Hermes for matching (it matches on `agentTaskId`); the pending Mark bubble carries the `agentTaskId`. If you prefer to thread the operator message id through, capture it from `insertOperatorMessage`'s return and pass it — optional.
...
git commit -m "feat(mark-chat): server actions for send/rename/archive/poll"
```
...
**Files:**
- Create: `src/app/mark/page.tsx`
- Modify: `src/app/_data/growth-engine.ts`

- [ ] **Step 1: Repoint the nav**

In `src/app/_data/growth-engine.ts`, change the Mark entry:
...
Skip running build here; it will fail until `mark-chat.tsx` exists. Proceed to Task 9, then build at Task 9's verify step.

- [ ] **Step 4: Commit**

```bash
git add src/app/mark/page.tsx src/app/_data/growth-engine.ts
git commit -m "feat(mark-chat): /mark page + repoint Mark nav"
```
...
**Files:**
- Create: `src/app/mark/_components/thread-sidebar.tsx`
- Create: `src/app/mark/_components/mark-chat.tsx`
...
export function ThreadSidebar({
  ...
  return (
    <aside className="flex min-h-0 flex-col gap-3 lg:w-72 lg:shrink-0">
      <Link href="/mark" className={theme.button ? "" : ""} aria-label="Start a new chat with Mark">
...
```bash
git add src/app/mark/_components/thread-sidebar.tsx src/app/mark/_components/mark-chat.tsx
git commit -m "feat(mark-chat): chat shell + thread sidebar"
```
...
**Files:**
- Create: `src/app/mark/_components/message-list.tsx`
...
git commit -m "feat(mark-chat): message rows + thinking indicator"
```
...
**Files:**
- Create: `src/app/mark/_components/composer.tsx`
...
Run: `pnpm build && pnpm lint`
Expected: PASS. The `/mark` route appears in the build output. Fix any unused-import or type errors surfaced.

- [ ] **Step 3: Commit**

```bash
git add src/app/mark/_components/composer.tsx
git commit -m "feat(mark-chat): composer with @-mention popover and chips"
```
...
Run: `pnpm test`
Expected: all tests pass (including the new `mark-chat` domain tests).

Run: `pnpm build && pnpm lint`
Expected: both clean; `/mark` and `/api/v1/hermes/messages` present in build output.

- [ ] **Step 3: Manual check (`pnpm dev`)**

- Visit `/mark` (with Supabase configured): the "Mark" nav item is highlighted; empty state shows; type a message → your row appears + "Mark is thinking…" pulses; a new thread appears in the sidebar; URL becomes `/mark?c=<id>`.
- Simulate Mark's reply (replace the token + a real `agentTaskId` from the pending row):
  ```bash
  curl -X POST http://localhost:3000/api/v1/hermes/messages \
    -H "Authorization: Bearer $HERMES_AGENT_API_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"agentTaskId":"<task-id>","body":"Here is what I found."}'
  ```
  Within ~2.5s the thinking row flips to Mark's reply.
- Type `@` → mention popover filters; pick one → chip appears; send → chip renders in the message and links to the record.
- Without Supabase configured: `/mark` shows the read-only preview.
...
```bash
git add DESIGN.md
git commit -m "docs(design): document the Mark chat surface"
```
...
- **Spec coverage:** data model → Task 1; domain (titles/validation/mentions) → Task 2; persistence → Task 3; enqueue → Task 4; mention catalog → Task 5; callback API → Task 6; actions (send/rename/archive/poll) → Task 7; page + nav → Task 8; shell + sidebar → Task 9; message rows + thinking → Task 10; composer + mentions → Task 11; degradation/empty states throughout (Tasks 8, 10, 11); docs → Task 12. Reference cards: Mark message `metadata` is stored; rendering a dedicated reference card is deferred (mentions render as linked chips, which covers the linking need) — noted as a fast-follow, not a gap that blocks the feature.
- **Placeholder scan:** none — every code step is complete.
- **Type consistency:** `MarkMention`/`MentionType` (Task 2) used across Tasks 3/5/7/10/11; `MarkConversation`/`MarkMessage`/`MarkMessageStatus` (Task 3) used in Tasks 6/7/9/10/11; `MentionGroup` (Task 5) used in Tasks 8/9/11; `SendMessageState` (Task 7) used in Task 11; `enqueueMarkChatTask`/`findPendingMessageByTask`/`completeMarkMessage`/`failMarkMessage` defined and consumed consistently. `getThreadMessagesAction(conversationId: string)` signature matches its call in Task 9.
...
- Rich "reference cards" for actions Mark proposes (links into the approval pipeline).
