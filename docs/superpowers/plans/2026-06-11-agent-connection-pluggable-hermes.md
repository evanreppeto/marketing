# Agent Connection — Pluggable Hermes Implementation Plan
...
- Modify every `src/app/api/v1/hermes/**/route.ts` — swap `checkBearerToken(req,"HERMES_AGENT_API_TOKEN")` → `checkAgentBearer(req)`.
- Modify `src/lib/mark-chat/agent-config.ts` + `notify.ts` — source config/secret from the resolver.
- Create `src/app/settings/agent-panel.tsx` + `src/app/settings/agent-actions.ts`; modify `settings-sections.ts` + `page.tsx`.
- Modify `src/app/mark/actions.ts` (`getMarkAgentStatusAction` health) — additive.
...
## Task 5: Swap Hermes routes to checkAgentBearer

**Files:**
- Modify: every `src/app/api/v1/hermes/**/route.ts` that calls `checkBearerToken(request, "HERMES_AGENT_API_TOKEN")` (NOT the lead-intake route, which keeps `checkBearerToken`).

- [ ] **Step 1: Find call sites**

Run: `grep -rln 'checkBearerToken(request, "HERMES_AGENT_API_TOKEN")' src/app/api/v1/hermes`
...
Change `const auth = checkBearerToken(request, "HERMES_AGENT_API_TOKEN");` → `const auth = await checkAgentBearer(request);`
...
Run: `pnpm vitest run src/app/api/v1/hermes`
Expected: PASS — existing `ping`, `tasks`, `messages`, `health`, `drafts` tests green (they set `HERMES_AGENT_API_TOKEN`, which the env branch still honors).

- [ ] **Step 4: Typecheck + commit**

```bash
pnpm exec tsc --noEmit
git add src/app/api/v1/hermes
git commit -m "refactor(agent): hermes routes authenticate via checkAgentBearer"
```
...
describe("resolveWebhookSecret", () => {
  beforeEach(() => { delete process.env.MARK_WEBHOOK_SECRET; });
...
/**
 * Resolve the outbound HMAC signing secret: env MARK_WEBHOOK_SECRET wins; else
...
**Files:**
- Modify: `src/lib/mark-chat/notify.ts` (re-read first), `src/lib/mark-chat/agent-config.ts`
...
Note: these become `async`. Update callers — `grep -rn "markAgentKeys()\|getMarkDisplayName()\|isMarkRunnerConfigured()" src` — to `await` them (notably `src/app/mark/actions.ts:getMarkAgentStatusAction`, handled in Task 9).
...
export async function notifyMarkWebhook(payload: MarkNotifyPayload): Promise<boolean> {
  ...
  if (!url) return false;
  if (!conn.enabled) return false; // operator kill-switch (env MARK_WEBHOOK_DISABLED or DB)
...
- [ ] **Step 3: Verify mark-chat tests**

Run: `pnpm vitest run src/lib/mark-chat`
...
```bash
pnpm exec tsc --noEmit
git add src/lib/mark-chat/notify.ts src/lib/mark-chat/agent-config.ts
...
**Files:**
- Modify: `src/app/mark/actions.ts` (re-read first) — update `getMarkAgentStatusAction`; add `testAgentConnectionAction`.
...
Update `MarkConnection` (`src/app/mark/_components/mark-connection.tsx`) only if you want to show "last seen" — optional; the `attached`/`name` fields it already reads are unchanged, so this is non-breaking.
...
- [ ] **Step 3: Wire `recordAgentSeen` into the ping route** (`src/app/api/v1/hermes/ping/route.ts`): after a successful `checkAgentBearer`, call `await recordAgentSeen();` before returning 200 (so any agent call-in stamps health). Import from `@/lib/agent/health`.

- [ ] **Step 4: Typecheck + commit**

```bash
pnpm exec tsc --noEmit
git add src/app/mark/actions.ts src/app/api/v1/hermes/ping/route.ts src/app/mark/_components/mark-connection.tsx
...
import { testAgentConnectionAction } from "@/app/mark/actions";
...
import type { AgentTestResult } from "@/app/mark/actions";
...
- **Wake (app → agent):** `POST {webhookUrl}`, body `{ "type": "mark_chat_message", messageId, conversationId, agentTaskId, message, mentions, operator, route, mode, command?, attachments? }` (from `MarkNotifyPayload` in `src/lib/mark-chat/notify.ts`). Header `X-Webhook-Signature: <hex>` = `HMAC-SHA256(rawBody, signingSecret)`. Respond 2xx to ack.
- **Auth (agent → app):** `Authorization: Bearer <token>` (env `HERMES_AGENT_API_TOKEN` or an app-issued `sk_live_…` token).
- **Inbound endpoints** (reference each route's method/path/body/response): `GET /api/v1/hermes/ping`; tasks `claim`/`complete`/`block`/`log`; `POST /api/v1/hermes/messages`; `POST /api/v1/hermes/messages/{agentTaskId}/steps`; drafts; approvals.
- **Reply contract:** post the reply to `/api/v1/hermes/messages` settling `agentTaskId`; emit steps best-effort.
...
- [ ] **Step 3: Confirm back-compat** — with `HERMES_AGENT_API_TOKEN` set and no Supabase, `GET /api/v1/hermes/ping` with that bearer still returns 200 (env path in `checkAgentBearer`); `resolveAgentConnection()` returns env values.
