# Arc Action Cards Implementation Plan (Plan 2 of 2)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Render what Arc *did* as structured cards instead of prose — result cards (clickable record rows) and draft cards (preview + flags + inline Approve / Decline wired to the real campaign approval flow), driven by `message.metadata.actions[]`.

**Architecture:** App-side only, additive. A new `MarkActionCard` parsed from the existing `arc_messages.metadata` jsonb (like `steps`/`media`). Draft-card Approve/Decline call the campaign decision **library** (`decideAsset`) via a thin chat action; Request-revision links to the campaign page (it needs free text). Degrades gracefully: no `actions` → nothing renders.
...
**Spec:** `docs/superpowers/specs/2026-06-09-arc-chat-premium-experience-design.md` (Action cards section). **Predecessor:** Plan 1 (premium states & composer) — already shipped.
...
- Modify `src/domain/arc-chat.ts` — `MarkActionCard` (+ sub-types) + `parseActions`; pure, exported via the `@/domain` barrel.
- Modify `src/domain/__tests__/` — new `arc-actions.test.ts` for `parseActions`.
- Modify `src/lib/arc-chat/persistence.ts` — `actions: MarkActionCard[]` on `MarkMessage`; parse in `toMessage`.
- Modify `src/app/arc/_components/use-thread-poll.ts` — `sameMessages` compares `actions.length`.
- Modify `src/app/arc/actions.ts` — `decideCampaignDraftAction(formData)` calling `decideAsset`.
- Create `src/app/arc/_components/action-card.tsx` — renders a `MarkActionCard`.
- Modify `src/app/arc/_components/message-list.tsx` — render `message.actions` above References.
- Modify `docs/arc-worker-contract-premium.md` — add the `actions[]` payload schema.
- Fix `MarkMessage` literals (composer temp, arc-chat optimistic, test fixtures) to include `actions: []`.
...
- Test: `src/domain/__tests__/arc-actions.test.ts` (create)

- [ ] **Step 1: Write the failing test** — `src/domain/__tests__/arc-actions.test.ts`:
...
Run: `pnpm vitest run src/domain/__tests__/arc-actions.test.ts`
Expected: FAIL — `parseActions` not exported.

- [ ] **Step 3: Implement** — append to the END of `src/domain/arc-chat.ts`:
...
Run: `pnpm vitest run src/domain/__tests__/arc-actions.test.ts`
Expected: PASS (4 tests). (`@/domain` re-exports via `export * from "./arc-chat"`, so `MarkActionCard`/`parseActions` are auto-surfaced — verify the barrel is `export *`; if it's a named list, add them.)

- [ ] **Step 5: Commit**

```bash
git add src/domain/arc-chat.ts src/domain/__tests__/arc-actions.test.ts
git commit src/domain/arc-chat.ts src/domain/__tests__/arc-actions.test.ts -m "feat(arc-chat): MarkActionCard types + parseActions"
```
...
- Modify: `src/app/arc/_components/use-thread-poll.ts`
- Fix literals: `src/app/arc/_components/composer.tsx`, `src/app/arc/_components/arc-chat.tsx`, `src/app/arc/_components/use-thread-poll.test.ts`

- [ ] **Step 1: persistence — import + type + parse.** In `src/lib/arc-chat/persistence.ts`:
- Add `MarkActionCard` and `parseActions` to the existing `@/domain` import (the file already imports `parseMedia`, `parseMentions`, `MarkMedia`, `ArcMention` from `@/domain`).
...
- [ ] **Step 2: poll equality.** In `src/app/arc/_components/use-thread-poll.ts`, in `sameMessages`, add an `actions.length` comparison to the per-message check (so a reply gaining cards re-renders). Change the `if (...)` condition to also include:
```typescript
...
- `src/app/arc/_components/composer.tsx` — the `tempMessage` helper (after `feedback: null,` / `steps: [],`).
- `src/app/arc/_components/arc-chat.tsx` — the optimistic bubble in `handleRegenerate` (after `feedback: null,`).
- `src/app/arc/_components/use-thread-poll.test.ts` — the `msg()` base literal (after `feedback: null,` if present, else after `steps: []`).
Run `grep -rn "feedback: null" src/app/arc` to find them all.

- [ ] **Step 4: Verify**

Run: `pnpm lint && pnpm vitest run src/app/arc src/lib/arc-chat`
Expected: PASS (no missing-property errors on `MarkMessage` literals; poll test still green).

- [ ] **Step 5: Commit**

```bash
git commit src/lib/arc-chat/persistence.ts src/app/arc/_components/use-thread-poll.ts src/app/arc/_components/composer.tsx src/app/arc/_components/arc-chat.tsx src/app/arc/_components/use-thread-poll.test.ts -m "feat(arc-chat): actions[] on MarkMessage + poll equality"
```
...
```bash
git commit src/app/arc/actions.ts -m "feat(arc-chat): decideCampaignDraftAction for action-card approval"
```
...
**Files:**
- Create: `src/app/arc/_components/action-card.tsx`
...
git commit src/app/arc/_components/action-card.tsx -m "feat(arc-chat): ActionCard component (result + draft w/ inline approval)"
```
...
- [ ] **Step 1: import + render.** In `src/app/arc/_components/message-list.tsx`:
- Add: `import { ActionCard } from "./action-card";`
- In the Arc/system branch of `Message`, render cards for non-pending messages — add this directly BEFORE the `{!pending ? <References .../> : null}` line:
```tsx
...
```bash
git commit src/app/arc/_components/message-list.tsx -m "feat(arc-chat): render action cards in Arc replies"
```
...
**Files:**
- Modify: `docs/arc-worker-contract-premium.md`

- [ ] **Step 1: Append a section** to `docs/arc-worker-contract-premium.md`:
...
```bash
git commit docs/arc-worker-contract-premium.md -m "docs(arc-chat): action-card actions[] payload contract"
```
...
- [ ] **Step 1:** `pnpm vitest run && pnpm lint` — all green (incl. new `arc-actions` tests).
