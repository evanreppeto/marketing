# Mark Experience — Phase 2 (Live Step-by-Step Activity) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans. Steps use checkbox (`- [ ]`).

**Goal:** While a Mark reply is pending, show a live checklist of what Mark is actually doing (steps fill in as he works), resolving into his final answer.

**Architecture:** Steps live on the pending message's existing `metadata.steps` jsonb (no new table). A new `POST /api/v1/hermes/messages/{agentTaskId}/steps` lets Mark's worker append/advance steps; the existing thread polling renders them as a timeline. Additive to the live chat system.

**Tech Stack:** Next.js 16 + TS + Supabase + vitest.

> **⚠️ BUILD-COORDINATION:** the chat backend (`src/lib/mark-chat/**`, `src/app/api/v1/hermes/messages/**`, `src/app/mark/_components/**`) is under active parallel development (webhook claim, "fast" route). Re-read each file immediately before editing; keep changes additive; small commits. Backend tasks (1–3) are low-collision; UI tasks (4–5) are the hot ones — coordinate.

---

## File map
- Modify `src/lib/mark-chat/persistence.ts` — `MarkStep` type, `steps` on `MarkMessage`, parse in `toMessage`, `appendMarkStep()`.
- Create `src/lib/mark-chat/steps.test.ts` — step-merge unit tests.
- Create `src/app/api/v1/hermes/messages/[agentTaskId]/steps/route.ts` — append-step endpoint.
- Modify `src/app/mark/_components/message-list.tsx` — ActivityTimeline for pending steps + collapsed trace on complete.
- Modify `src/app/mark/_components/mark-chat.tsx` — `sameMessages` must compare steps so the poll updates the timeline.
- Deliverable: update Mark's `mark-chat-responder` skill to emit steps.

---

## Task 1: MarkStep type + parse steps in toMessage

**File:** `src/lib/mark-chat/persistence.ts` (re-read first)
...
- [ ] **Step 3: Verify** `pnpm lint` + `pnpm vitest run src/lib/mark-chat` green (any code constructing `MarkMessage` literals must add `steps: []` — there are none in app code, but check tests).

## Task 2: appendMarkStep persistence (TDD)

**Files:** `src/lib/mark-chat/persistence.ts`; Test `src/lib/mark-chat/steps.test.ts`
...
- [ ] **Step 2: Run to verify fail** — `pnpm vitest run src/lib/mark-chat/steps.test.ts`.
...
export async function appendMarkStep(
  ...
  const { error: upErr } = await client.from("mark_messages").update({ metadata: { ...meta, steps: nextSteps } }).eq("id", data.id);
  assertOk("mark_messages step update", upErr);
  return true;
}
```

- [ ] **Step 4: Run to verify pass** — `pnpm vitest run src/lib/mark-chat/steps.test.ts` (3 passed).
- [ ] **Step 5: Commit** — `git add src/lib/mark-chat/persistence.ts src/lib/mark-chat/steps.test.ts && git commit -m "feat(mark-chat): activity steps on pending messages (mergeStep + appendMarkStep)"`

## Task 3: Append-step endpoint

**File:** Create `src/app/api/v1/hermes/messages/[agentTaskId]/steps/route.ts` (mirror the existing messages route's auth/guards)
  ...
  if (!auth.ok) {
    return NextResponse.json(
      auth.reason === "not_configured"
        ? { ok: false, status: "not_configured", message: "Set HERMES_AGENT_API_TOKEN first." }
...
**File:** `src/app/mark/_components/message-list.tsx`

- [ ] When a Mark message is `pending` and has `steps.length > 0`, render a timeline: each step row shows `✓` (done) or a small spinner (`running`) + the label, newest building downward, with the existing "Waiting for Mark to reply…" line as the fallback when there are no steps yet. When the message is `complete`, render the body and (if steps exist) a small collapsed "Show what Mark did" `<details>` listing the done steps. Keep the app's tokens/classes.

## Task 5: Poll detects step changes *(HOT FILE — re-read first, coordinate)*

**File:** `src/app/mark/_components/mark-chat.tsx`
...
## Task 6: Mark contract — emit steps *(deliverable for Mark's Mac)*

- [ ] Update `marketing-classifier-agent/mark-skills/mark-chat-responder/SKILL.md`: before each meaningful action, `POST {APP_URL}/api/v1/hermes/messages/{agentTaskId}/steps` with `{ "label": "Searching Meta Ad Library", "status": "running" }`, then `{ ..., "status": "done" }` when finished; then post the final reply as today. Steps are best-effort (a failed step POST never blocks the reply). Outbound stays locked.

---

## Self-Review
- **Spec coverage:** steps on message metadata (T1), append endpoint + merge (T2–3), timeline UI (T4), live polling (T5), Mark contract (T6). ✓
