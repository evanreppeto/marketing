# Arc Chat Realtime Push Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Give Arc chat sub-second replies by having the Mac mini runner subscribe to Supabase Realtime `INSERT`s on `agent_tasks` (outbound socket, no tunnel), replacing the dead ephemeral-tunnel webhook push.

**Architecture:** The app already inserts a queued `agent_tasks` row on every operator message. A new Mac mini component opens an outbound WebSocket to Supabase, receives that insert in sub-second time, claims the task with a compare-and-set, runs Arc's existing turn, and POSTs the reply back through the already-working app API. The poller stays as the safety net; everything is idempotent so the two paths never double-answer.
...
- `arc-runner/arc_chat_core.py` — **create**: pure, unit-tested helpers — `row_to_message`, `is_chat_task`, `extract_record`, `build_claim_request`, `claim_won`, `claim_task`.
- `arc-runner/test_mark_chat_core.py` — **create**: stdlib `unittest` tests for the pure helpers.
- `arc-runner/realtime_subscriber.py` — **create**: thin glue wiring realtime-py → core → the configured worker module.
- `arc-runner/com.bsr.arc-realtime.plist` — **create**: `launchd` job (auto-start + restart).
- `arc-runner/requirements.txt` — **modify**: add `realtime`.
- `arc-runner/.env.example` — **modify**: add `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ARC_WORKER_MODULE`.
- `arc-runner/README.md` — **modify**: document the realtime subscriber + phasing.
- `scripts/diagnose-arc-connection.mjs` — **modify**: finalize as the supported diagnostic.
- `package.json` — **modify**: add `diagnose:arc` script.
...
```sql
-- Broadcast agent_tasks inserts over Supabase Realtime so the Arc runner
...
git commit -m "feat(realtime): publish agent_tasks for Arc chat instant delivery"
```
...
**Files:**
- Create: `arc-runner/arc_chat_core.py`
- Test: `arc-runner/test_mark_chat_core.py`

- [ ] **Step 1: Write the failing tests**

```python
# arc-runner/test_mark_chat_core.py
import unittest
...
class IsChatTask(unittest.TestCase):
    def test_true_for_chat_task(self):
        self.assertTrue(is_chat_task({"task_type": "arc_chat_message"}))
...
Run: `cd arc-runner && python3 -m unittest test_mark_chat_core -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'mark_chat_core'`.

- [ ] **Step 3: Write the helpers**

```python
# arc-runner/arc_chat_core.py
...
a live socket or network. Mirrors toInboxItem() in src/lib/arc-chat/inbox.ts.
"""
import json
...
def is_chat_task(record):
    ...
    restricts task_type=arc_chat_message, but updates/replays shouldn't slip through)."""
    return isinstance(record, dict) and record.get("task_type") == "arc_chat_message"
...
```bash
git add arc-runner/arc_chat_core.py arc-runner/test_mark_chat_core.py
git commit -m "feat(arc-runner): pure helpers for realtime row→message mapping"
```
...
**Files:**
- Modify: `arc-runner/arc_chat_core.py`
- Test: `arc-runner/test_mark_chat_core.py`
...
```bash
git add arc-runner/arc_chat_core.py arc-runner/test_mark_chat_core.py
git commit -m "feat(arc-runner): CAS claim helper for exactly-once chat handling"
```
...
**Files:**
- Create: `arc-runner/realtime_subscriber.py`
...
Run (after Task 6's install, or in the same venv): `cd arc-runner && python3 -c "import realtime_subscriber; print('import ok')"`
Expected: `import ok` (no syntax/import errors).

- [ ] **Step 3: Commit**

```bash
git add arc-runner/realtime_subscriber.py
git commit -m "feat(arc-runner): Supabase Realtime subscriber for instant chat"
```
...
**Files:**
- Modify: `arc-runner/requirements.txt`
- Modify: `arc-runner/.env.example`
- Modify: `arc-runner/README.md`

- [ ] **Step 1: Add the dependency**

Replace `arc-runner/requirements.txt` with:
...
Run: `cd arc-runner && python3 -m pip install -r requirements.txt`
Expected: `realtime` and `mcp` installed (or already satisfied).

- [ ] **Step 3: Extend .env.example**

Append to `arc-runner/.env.example`:
...
# Defaults to the reference poller; point it at Arc's real worker module if different.
ARC_WORKER_MODULE=poller
```

- [ ] **Step 4: Document it in README.md**

Insert after the "Run the poller" section in `arc-runner/README.md`:
...
```bash
git add arc-runner/requirements.txt arc-runner/.env.example arc-runner/README.md
git commit -m "docs(arc-runner): realtime subscriber setup + realtime dependency"
```
...
**Files:**
- Create: `arc-runner/com.bsr.arc-realtime.plist`

- [ ] **Step 1: Write the plist**

Replace `__ARC_RUNNER_DIR__` and `__PYTHON__` when installing (instructions in Step 2).
...
<dict>
  <key>Label</key>
  <string>com.bsr.arc-realtime</string>
  ...
  <string>__ARC_RUNNER_DIR__</string>
  ...
  <string>__ARC_RUNNER_DIR__/arc-realtime.log</string>
  <key>StandardErrorPath</key>
  <string>__ARC_RUNNER_DIR__/arc-realtime.err.log</string>
</dict>
</plist>
```
...
```bash
git add arc-runner/com.bsr.arc-realtime.plist arc-runner/README.md
git commit -m "feat(arc-runner): launchd job to auto-start the realtime subscriber"
```
...
**Files:**
- Modify: `scripts/diagnose-arc-connection.mjs`
- Modify: `package.json`
...
```json
"diagnose:arc": "node scripts/diagnose-arc-connection.mjs",
```

- [ ] **Step 3: Run it**

Run: `pnpm diagnose:arc`
Expected: prints `agent_connections` with `last_status: "ok"`, the `arc` agent row, token list, and recent chat tasks — no `ERROR:` lines.

- [ ] **Step 4: Commit**

```bash
git add scripts/diagnose-arc-connection.mjs package.json
git commit -m "chore: supported pnpm diagnose:arc connection check"
```
...
On a host with the env set (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `APP_BASE_URL`, `ARC_AGENT_API_TOKEN`):
Run: `cd arc-runner && set -a; . ./.env; set +a; python3 realtime_subscriber.py`
Expected: logs `subscription: SUBSCRIBED — listening for chat tasks`.

- [ ] **Step 2: Trigger a real message**

Send a message to Arc from the app UI (`/arc`), OR insert a queued task directly against prod with the service key:

```bash
curl -sS -X POST "$SUPABASE_URL/rest/v1/agent_tasks" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" -H "authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  -H "content-type: application/json" -H "prefer: return=representation" \
  -d '{"agent_id":"<arc agent id from diagnose:arc>","status":"queued","priority":"high","objective":"Realtime smoke test — reply OK","task_type":"arc_chat_message","source_type":"mark_conversation","source_id":"<a real conversation id or omit if UI-sent>","metadata":{"conversation_id":"<conv>","requested_by":"diag","source":"mark_chat","model_route":"fast","mode":"ask","outbound_locked":true}}'
```
...
Run: `pnpm diagnose:arc`
Expected: the test task shows `status` no longer `queued` (it's `completed`/`running`→settled); for a UI-sent message, the thread shows Arc's reply.

- [ ] **Step 4: Confirm no double-answer**

With the subscriber running AND the poller firing, send one message. Expected: exactly one Arc reply (the loser path's claim returns 0 rows / the duplicate reply 404s). Verify only one `arc` message row was added for that turn via `pnpm diagnose:arc`.
...
- **Service-role key handling:** it lives only in the Mac mini's `arc-runner/.env`. Do not commit it; `.env` is already gitignored (only `.env.example` is tracked).
...
- **Vercel:** confirm `ARC_AGENT_API_TOKEN` is set in the Vercel project env (the subscriber's `deliver()` posts there). The migration must be applied to the prod DB manually (per repo convention).
