# Mark UI Redesign — Plan 1: Visual Elevation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the Mark chat feel like a full-bleed, professional, *alive* chat tool — without changing any backend or send behavior.
...
**This is Plan 1 of 4** for the Mark UI redesign (`docs/superpowers/specs/2026-06-10-mark-ui-redesign-design.md` §5). Later plans: 2) composer simplification (drop mode), 3) structured slash commands, 4) Agent Port + connection states. This plan must keep the app building, linting, and tests green.

---

## Context

- `src/app/mark/_components/mark-chat.tsx` — the chat shell. **Line ~237** wraps everything in `rounded-xl border border-[var(--border-panel)] bg-[var(--surface-panel)] shadow-[var(--elev-panel)]` — *this is the floating card*.
- The Mark route renders inside `ConsoleFrame`'s content section, which has padding (`theme.shell.content`: `px-4 py-4 sm:px-6 lg:px-8 lg:py-5 xl:px-10`, and `lg:h-screen lg:overflow-y-auto`). Full-bleed = the Mark root cancels that padding with negative margins and fills height.
- `src/app/mark/_components/message-list.tsx` — already renders operator bubbles, Mark messages with `MarkAvatar` (breathes via `avatar-breathe`), `PendingBlock` (steps + streaming caret + `mark-shimmer`), `StepTrace`, action cards, references, hover actions. We restyle, not rebuild.
...
- Modify: `src/app/mark/_components/mark-chat.tsx` — remove card wrapper, full-bleed breakout.
- Modify: `src/app/mark/_components/message-list.tsx` — presence avatar, work-timeline, streaming fix, message polish, empty state.
...
**Files:** Modify `src/app/mark/_components/mark-chat.tsx` (the outer wrapper around line 236–240, and the root at line 234).
...
Run: `pnpm build` → clean. Then `pnpm dev`, open `/mark`: the chat fills edge-to-edge with no rounded card border and no outer padding gap; the thread sidebar meets the app rail; the composer anchors at the bottom.

- [ ] **Step 5: Commit**

```bash
git add src/app/mark/_components/mark-chat.tsx
git commit -m "feat(mark): full-bleed chat surface (remove floating card)"
```
...
**Files:** Modify `src/app/mark/_components/message-list.tsx` — the `MarkAvatar` component (lines 71–83).

- [ ] **Step 1: Restyle the avatar to a dark tile with a gold mark and a single breathing ring**
...
- [ ] **Step 2: Add the `mark-ring` keyframe** to `src/app/globals.css` (near the other Mark keyframes):
...
git commit -m "feat(mark): calm breathing presence avatar"
```

---

## Task 3: Work-timeline — Mark shows his work

**Files:** Modify `src/app/mark/_components/message-list.tsx` — `StepRow` (85–99), `PendingBlock` step section (107–113), and `StepTrace` (149–165).
...
function StepTrace({ steps }: { steps: MarkStep[] }) {
  return (
    <details className="mt-3 text-xs text-[var(--text-muted)]">
      <summary className="cursor-pointer select-none font-mono text-[11px] hover:text-[var(--text-secondary)]">What Mark did · {steps.length}</summary>
...
```bash
pnpm build
git add src/app/mark/_components/message-list.tsx
git commit -m "feat(mark): work-timeline — Mark's steps render as a connected sequence"
```
...
**Files:** Modify `src/app/mark/_components/message-list.tsx` — `PendingBlock` no-body branch (124–134).

- [ ] **Step 1: Replace the `mark-shimmer` gradient-text block** (the `!hasSteps` branch, lines 124–134) with solid label + skeleton bars (no `background-clip:text`):
...
.mark-skel::after {
  ...
  animation: mark-skel-slide 1.5s ease-in-out infinite;
}
@keyframes mark-skel-slide { 100% { transform: translateX(100%); } }
@media (prefers-reduced-motion: reduce) { .mark-skel::after { animation: none; } }
```

- [ ] **Step 3:** Confirm `mark-shimmer` / `mark-shimmer-bar` are no longer referenced in `message-list.tsx` (grep). Leave the CSS classes in `globals.css` (other code may use them); we just stop using the gradient-text variant here.
...
pnpm build
git add src/app/mark/_components/message-list.tsx src/app/globals.css
git commit -m "feat(mark): solid streaming text + skeleton (drop gradient-text tell)"
```
...
**Files:** Modify `src/app/mark/_components/message-list.tsx` — operator bubble (293–301), name line (313–321), `MentionChips`/`References` (167–202), empty state (400–415).
...
- [ ] **Step 2: Mark name line uses serif** (line 315) — replace `font-display` with serif and bump to the name treatment:

```tsx
            <span className="font-serif text-[13px] font-semibold text-[var(--text-primary)]" style={{ fontFamily: "var(--font-serif)" }}>Mark</span>
```
...
pnpm build
git add src/app/mark/_components/message-list.tsx
git commit -m "feat(mark): message + reference polish (gold as punctuation)"
```
...
**Files:** Modify `src/app/mark/_components/message-list.tsx` — the `messages.length === 0` block (400–415).
...
pnpm build
git add src/app/mark/_components/message-list.tsx
git commit -m "feat(mark): grounded, teaching empty state"
```
...
- [ ] **Step 1:** `pnpm test` → all pass (the Mark tests `slash-commands.test.ts`, `relative-time.test.ts`, `use-thread-poll.test.ts` are untouched and stay green).
- [ ] **Step 2:** `pnpm lint` → clean.
- [ ] **Step 3:** `pnpm build` → clean.
- [ ] **Step 4: Visual smoke** — `pnpm dev`, open `/mark`:
  - Full-bleed, no floating card, composer anchored at the bottom.
  - Send a message (or view a thread): Mark's avatar breathes once-per-cycle; steps render as a connected timeline; streaming shows solid text + caret (no shimmering gradient text); references are quiet chips.
  - Operator bubble is subtle; serif "Mark" name; hover reveals Copy/Regenerate/feedback.
