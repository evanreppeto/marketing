# Arc — Save & Promote + Chat Settings Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Let the operator ⭐-save Arc's chat outputs (media, draft cards, message text as angles), revisit them in a Saved view, attach a project/campaign to a chat via a settings gear, and promote a saved item into the existing `/campaigns` approval flow (attached to an existing campaign or a newly created one).

**Architecture:** Pure additive feature. New `arc_saved_items` table + a `campaign_id` link on `arc_conversations`. A new `src/lib/arc-chat/saved.ts` persistence module and two new `src/lib/campaigns/create.ts` helpers reuse the existing approval pipeline (`campaign_assets` + `approval_items` + `campaign_events`). New server actions in `src/app/arc/actions.ts`. UI: ⭐ on media/draft/message, a `/arc/saved` page, a Promote dialog, and a chat-settings popover. No change to the approval state machine.
...
**Reference spec:** `docs/superpowers/specs/2026-06-10-arc-saved-promote-design.md`
**DESIGN.md guardrails (§8):** no emoji, no glow/gradient decoration, no purple/neon, no nested cards, reduced-motion safe. Arc surface may use the richer visuals already established.

---

## File Structure

- `supabase/migrations/20260610170000_arc_saved_items.sql` — **create**: `arc_saved_items` table + `arc_conversations.campaign_id` column.
- `src/lib/arc-chat/persistence.ts` — **modify**: add `campaignId` to `MarkConversation`/`ConversationRow`/`CONVERSATION_COLUMNS`/`toConversation`; add `assignConversationToCampaign`.
- `src/lib/arc-chat/saved.ts` — **create**: `SavedItem` type + CRUD (`saveItem`, `listSavedItems`, `removeSavedItem`, `findSavedBySource`, `markPromoted`).
- `src/lib/arc-chat/saved.test.ts` — **create**: unit tests for `saved.ts`.
...
- `src/app/arc/actions.ts` — **modify**: `saveMarkItemAction`, `unsaveMarkItemAction`, `promoteSavedItemAction`, `attachCampaignForm`.
- `src/app/arc/_components/save-star.tsx` — **create**: reusable ⭐ toggle button.
- `src/app/arc/_components/message-media.tsx` — **modify**: ⭐ on each media item.
- `src/app/arc/_components/action-card.tsx` — **modify**: ⭐ on draft cards.
- `src/app/arc/_components/message-list.tsx` — **modify**: "Save as angle" in the Arc message action bar; thread `MarkSavedContext` provider for saved keys.
- `src/app/arc/page.tsx` — **modify**: load saved-source keys + pass to `MarkChat`.
- `src/app/arc/_components/arc-chat.tsx` — **modify**: thread saved keys down; add settings gear + chip; pass `activeCampaignId`.
- `src/app/arc/_components/chat-settings.tsx` — **create**: settings popover (project + campaign attach).
- `src/app/arc/saved/page.tsx` — **create**: Saved view (server component).
- `src/app/arc/saved/_components/saved-list.tsx` — **create**: client list + per-item actions.
- `src/app/arc/saved/_components/promote-dialog.tsx` — **create**: Existing/New campaign promote dialog.
- `src/app/arc/_components/thread-sidebar.tsx` — **modify**: add a "Saved" link by "Archived".
...
**Files:**
- Create: `supabase/migrations/20260610170000_arc_saved_items.sql`

- [ ] **Step 1: Write the migration**

```sql
-- Arc "Saved" pinboard: operator-starred chat outputs (media, draft cards, angles),
...
Run: `node -e "const s=require('fs').readFileSync('supabase/migrations/20260610170000_arc_saved_items.sql','utf8'); if(!/create table/.test(s)||!/add column/.test(s)) throw new Error('migration missing statements'); console.log('ok')"`
Expected: `ok`

- [ ] **Step 3: Commit**

```bash
git add supabase/migrations/20260610170000_arc_saved_items.sql
git commit -m "feat(arc): migration for arc_saved_items + conversation campaign_id"
```
...
Append to `src/lib/arc-chat/persistence.test.ts` (inside the file, after existing tests; reuse its existing Supabase mock helper — check the top of the file for how it builds a fake client and a conversation row, and mirror that). Add:
...
Run: `pnpm test src/lib/arc-chat/persistence.test.ts`
...
In `src/lib/arc-chat/persistence.ts`:
...
```bash
git add src/lib/arc-chat/persistence.ts src/lib/arc-chat/persistence.test.ts
git commit -m "feat(arc): conversation campaignId read-model + assignConversationToCampaign"
```
...
**Files:**
- Create: `src/lib/arc-chat/saved.ts`
- Test: `src/lib/arc-chat/saved.test.ts`
...
Run: `pnpm test src/lib/arc-chat/saved.test.ts`
Expected: PASS (4 tests).

- [ ] **Step 4: Commit**

```bash
git add src/lib/arc-chat/saved.ts src/lib/arc-chat/saved.test.ts
git commit -m "feat(arc): saved-items persistence (saved.ts) + tests"
```
...
export async function createCampaignShell(input: CreateCampaignShellInput): Promise<{ campaignId: string }> {
  ...
  await client.from("campaign_events").insert({
    ...
    detail: "created from Arc saved item",
  });
...
export async function promoteAssetToCampaign(input: PromoteAssetInput): Promise<{ assetId: string }> {
  ...
  await client.from("campaign_events").insert({
    ...
    actor: input.operator,
    detail: "promoted from Arc saved",
  });
...
- Test: `src/app/arc/actions.promote.test.ts`

> The file is already `"use server"` with `requireOperator()`, `getOperatorActor()`, `isSupabaseAdminConfigured()`, `getSupabaseAdminClient()`, and `revalidatePath` in scope. Add imports from `@/lib/arc-chat/saved`, `@/lib/campaigns/create`, `@/domain` (`OFFICIAL_PERSONA_MAPPINGS`, `RESTORATION_FOCUS_VALUES`), and `assignConversationToCampaign` from `@/lib/arc-chat/persistence`.

- [ ] **Step 1: Add a pure validator + the actions**

Add to `src/app/arc/actions.ts`:
...
import { assignConversationToCampaign } from "@/lib/arc-chat/persistence";
...
export async function promoteSavedItemAction(
  ...
  const { assetId } = await promoteAssetToCampaign({
    ...
    campaignId,
    assetType,
    title: item.title ?? "Promoted from Arc",
    body: item.body,
...
Create `src/app/arc/actions.promote.test.ts`:
...
Run: `pnpm test src/app/arc/actions.promote.test.ts`
Expected: PASS (4 tests).
Run: `npx eslint src/app/arc/actions.ts`
Expected: clean (exit 0).

- [ ] **Step 4: Commit**

```bash
git add src/app/arc/actions.ts src/app/arc/actions.promote.test.ts
git commit -m "feat(arc): save/unsave/promote/attach-campaign server actions"
```
...
**Files:**
- Create: `src/app/arc/_components/save-star.tsx`
...
Run: `npx eslint src/app/arc/_components/save-star.tsx`
Expected: clean.

- [ ] **Step 3: Commit**

```bash
git add src/app/arc/_components/save-star.tsx
git commit -m "feat(arc): reusable SaveStar toggle"
```
...
**Files:**
- Modify: `src/app/arc/_components/message-media.tsx`
...
- [ ] **Step 3: "Save as angle"** — in `message-list.tsx`, in the Arc message action bar (the persistent copy/regenerate/feedback row), add:
...
Run: `npx eslint src/app/arc/_components/message-media.tsx src/app/arc/_components/action-card.tsx src/app/arc/_components/message-list.tsx`
Expected: clean.
...
```bash
git add src/app/arc/_components/message-media.tsx src/app/arc/_components/action-card.tsx src/app/arc/_components/message-list.tsx
git commit -m "feat(arc): SaveStar on media, draft cards, and message angles"
```
...
**Files:**
- Create: `src/app/arc/saved/page.tsx`
- Create: `src/app/arc/saved/_components/saved-list.tsx`
- Modify: `src/app/arc/_components/thread-sidebar.tsx`

- [ ] **Step 1: Saved page (server component)**

`src/app/arc/saved/page.tsx`:
...
export default async function MarkSavedPage() {
  ...
  if (!isSupabaseAdminConfigured()) {
    return (
      <>
        <PageHeader eyebrow="Arc" title="Saved" description="Items you star in chat live here, ready to promote into a campaign." backHref="/arc" backLabel="Back to chat" />
  ...
  return (
    <>
      <PageHeader eyebrow="Arc" title="Saved" description="Items you star in chat. Keep experimenting, or promote one into a campaign for approval." backHref="/arc" backLabel="Back to chat" />
      {items.length === 0 ? (
        <EmptyState title="Nothing saved yet" detail="In a chat with Arc, hit the star on a generated image, a draft, or a message to keep it here." />
      ) : (
...
`src/app/arc/saved/_components/saved-list.tsx`:
              ...
              <div key={item.id} className="signal-panel flex flex-col gap-2 p-3">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  {item.sourceConversationId ? (
                    <Link href={`/arc?c=${item.sourceConversationId}`} className="rounded-md px-2.5 py-1 text-xs font-medium text-[var(--text-secondary)] shadow-[inset_0_0_0_1px_var(--border-strong)] transition hover:text-[var(--text-primary)]">
                      Continue in chat
                    </Link>
                  ) : null}
...
Run: `npx eslint src/app/arc/saved/page.tsx src/app/arc/saved/_components/saved-list.tsx src/app/arc/_components/thread-sidebar.tsx`
Expected: clean.

- [ ] **Step 5: Commit**

```bash
git add src/app/arc/saved/ src/app/arc/_components/thread-sidebar.tsx
git commit -m "feat(arc): Saved view + sidebar link"
```
...
**Files:**
- Create: `src/app/arc/saved/_components/promote-dialog.tsx`
...
Run: `npx eslint src/app/arc/saved/_components/promote-dialog.tsx`
Expected: clean.

- [ ] **Step 3: Commit**

```bash
git add src/app/arc/saved/_components/promote-dialog.tsx
git commit -m "feat(arc): promote dialog (existing / new campaign)"
```
...
**Files:**
- Create: `src/app/arc/_components/chat-settings.tsx`
- Modify: `src/app/arc/_components/arc-chat.tsx`
- Modify: `src/app/arc/page.tsx`

- [ ] **Step 1: Pass campaign context into the chat** — in `src/app/arc/page.tsx`, the active conversation already loads via `getConversation`. Pass its `campaignId` and the operator's campaign list to `MarkChat`:
...
`src/app/arc/_components/chat-settings.tsx`:
...
import { cx } from "@/app/_components/theme";
import type { MarkProject } from "@/lib/arc-chat/persistence";
...
- [ ] **Step 3: Mount the gear + campaign chip in the header** — in `arc-chat.tsx`, in the header's right-side controls (where `MarkConnection` and `ThreadMenu` render, only when `activeId`), add the gear and, when a campaign is attached, a chip:
...
```
(`Link` is already imported in `arc-chat.tsx`. Add `activeCampaignId` and `campaigns` to the destructured props + type.)

- [ ] **Step 4: Build + lint**

Run: `npx eslint src/app/arc/_components/chat-settings.tsx src/app/arc/_components/arc-chat.tsx src/app/arc/page.tsx`
Expected: clean.

- [ ] **Step 5: Commit**

```bash
git add src/app/arc/_components/chat-settings.tsx src/app/arc/_components/arc-chat.tsx src/app/arc/page.tsx
git commit -m "feat(arc): chat settings gear (project + campaign attach) + header chip"
```
...
Run: `pnpm lint`
Expected: no new warnings in `src/app/arc/`, `src/lib/arc-chat/`, `src/lib/campaigns/`.
...
Run `pnpm dev`, then on `/arc`:
- Save a generated media item, a draft card, and a Arc message (angle); stars fill.
- `/arc/saved` lists all three groups with previews.
...
- Supabase-unconfigured: `/arc/saved` shows the connect-Supabase empty state; saving is disabled.
...
```bash
git add -A
git commit -m "chore(arc): save/promote guardrail polish"
```
