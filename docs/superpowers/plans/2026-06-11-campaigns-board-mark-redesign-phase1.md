# Campaigns × Board × Mark — Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the campaigns list page and the 7-tab campaign detail page with a calm, board-intertwined experience where a campaign is an overview and its work lives in Mark threads + board tasks — serving humans and Mark alike.

**Architecture:** One spine, three lenses. A campaign's *work* is `agent_tasks` rows linked by `campaign_id`; its *conversations* are `mark_conversations` rows linked by `campaign_id`. The list page gains an "Ask Mark to build" bar + a who-drives creation chooser + status cards. The detail page collapses 7 tabs into one Overview scroll (At a glance / The package with inline approve / What's live / Audience / Work lane / Threads / History) with an "Open in Mark" redirect. Phase 1 adds **no migration** — all needed columns exist. Additive only; outbound stays locked.
...
- `src/app/campaigns/_components/ask-mark-bar.tsx` — client; the "describe a campaign → Mark builds it" input.
...
- `src/app/campaigns/page.tsx` + `campaign-library.tsx` — cards + filters + ask-Mark bar.
...
Fetches the Mark conversations and board tasks linked to one campaign, for the Overview's Work lane + Threads panel. Split into a pure mapper (tested) + an I/O fetcher (guarded by Supabase config).
...
/** I/O: Mark conversations linked to this campaign, newest first. Mirror the
 *  column list + mapping of `listConversations` in lib/mark-chat/persistence.ts. */
export async function getCampaignThreads(
  ...
  return (data ?? []).map((r) => ({
    ...
    href: `/mark?conversation=${r.id}`,
  }));
}
```

> Note: confirm `/mark` reads `?conversation=<id>` to open a thread. Re-read `src/app/mark/page.tsx`; if it uses a different param (e.g. `?c=`), adjust `href` here and in Task 9's Threads panel.
...
`askMarkToBuildCampaignAction` is the headline intertwine: shell campaign → linked Mark conversation → operator's first message → queued board task → redirect into `/mark`. `handToMarkAction` queues a "continue building" directive for an existing campaign.
...
import { createConversation, assignConversationToCampaign, insertOperatorMessage } from "@/lib/mark-chat/persistence";
import { parseBuildPrompt, deriveCampaignName } from "./build-campaign";
```
 ...
 * task (campaign_directive) for Mark, then sends the operator into the chat.
 * Outbound stays locked — Mark drafts, the human approves.
 */
export async function askMarkToBuildCampaignAction(formData: FormData): Promise<void> {
  ...
  // Persona/focus are unknown at ask-time; Mark fills them in. Use neutral seeds
  ...
  redirect(`/mark?conversation=${conversation.id}`);
}
...
/** Ensure the Mark agent row exists and return its id. Mirrors `ensureMarkAgent`
...
> Note: re-read `createConversation` / `insertOperatorMessage` signatures in `lib/mark-chat/persistence.ts` and match argument shapes exactly (param object keys may differ — e.g. `insertOperatorMessage` may want `{ conversationId, body, mentions }` or positional args). Adjust the calls to the real signatures; keep the composition identical.
...
git commit -m "feat(campaigns): ask-Mark-to-build + hand-to-Mark actions"
```
...
**Files:**
- Create: `src/app/campaigns/_components/ask-mark-bar.tsx`

- [ ] **Step 1: Implement**

```tsx
// src/app/campaigns/_components/ask-mark-bar.tsx
...
/** The "describe a campaign → Mark builds it" entry on the list page. Server
  ...
  return (
    ...
    >
      ...
      <input
        ...
        type="text"
        maxLength={2000}
        placeholder="Describe a campaign and Mark will build it — e.g. “Storm response for flood-zone landlords this week”"
...
```bash
git add src/app/campaigns/_components/ask-mark-bar.tsx
git commit -m "feat(campaigns): ask-Mark build bar"
```
      ...
      <div className="flex h-16 gap-px bg-[var(--canvas-deep)]">
        {campaign.thumbnailUrl ? (
          ...
          <div className="grid h-16 w-full place-items-center text-[11px] text-[var(--text-muted)]">
            {campaign.lifecycle === "Drafting" ? "Mark is drafting assets…" : "No creative yet"}
          </div>
        )}
      </div>
...
## Task 8: List page — cards + filters + ask-Mark bar
...
import { AskMarkBar } from "./ask-mark-bar";
...
In `CampaignsHeader`, change the "Needs you" pill tone to `amber` (gold) and point the CTA at the chooser. Replace the `<Link href="/campaigns/new" …>＋ Ask Mark to build one</Link>` with:
...
```bash
pnpm exec tsc --noEmit
pnpm exec eslint src/app/campaigns/_components/campaign-library.tsx src/app/campaigns/_components/campaign-card.tsx src/app/campaigns/_components/ask-mark-bar.tsx src/app/campaigns/page.tsx
```
Expected: both clean.
...
git commit -m "feat(campaigns): list page — ask-Mark bar, filters, status cards"
```
...
- [ ] **Step 1: `at-a-glance.tsx`** — Mark's "why" in serif + facts.
...
import { theme, cx } from "@/app/_components/theme";
import { AssetThumb } from "@/app/mark/_components/asset-thumb";
...
> Note: confirm `MarkActionCard` required fields (from `@/domain`) — `rows`/`flags` are arrays, `kind` is `"draft"|"result"`. Match exactly so `toCard` typechecks. `CampaignMediaAsset` shape: pass `{ kind: "image", url }`; check `MarkMedia` requires only `kind` + `url` (it does, per `domain/mark-chat.ts`).
...
- [ ] **Step 5: `work-lane.tsx`** — the campaign's board tasks + "+ New task" + "Hand to Mark".
  ...
  return (
    <Panel>
      ...
      <div className="mt-3 flex flex-col gap-2">
        {tasks.length === 0 ? (
          <p className="text-[12px] text-[var(--text-muted)]">No active work. Hand it to Mark to keep building.</p>
        ) : tasks.map((t) => (
      ...
      <form action={handToMarkAction} className="mt-3">
        <input type="hidden" name="campaignId" value={campaignId} />
        <button type="submit" className="text-[11px] text-[var(--accent)]">+ Hand to Mark</button>
      </form>
    </Panel>
  );
}
...
- [ ] **Step 6: `threads-panel.tsx`** — Mark conversations under the campaign.
...
export function ThreadsPanel({ threads }: { threads: CampaignThread[] }) {
  return (
    <Panel>
      <p className={theme.text.eyebrow}>Mark threads</p>
      <div className="mt-3 flex flex-col gap-2">
        {threads.length === 0 ? (
...
export function CampaignWorkspace({ detail }: { detail: LiveCampaignWorkspace; dispatches: CampaignDispatch[] }) {
  ...
  // First Mark thread for this campaign opens via the markConversation linkage;
  // fall back to a campaign-scoped /mark deep link.
  const openInMark = `/mark?campaign=${campaign.id}`;
  return (
    <>
      <PageHeader
        eyebrow="Campaign"
        ...
        aside={
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Link href={openInMark} className={buttonClasses({ variant: "ghost", size: "sm" })}>Open in Mark</Link>
          </div>
        }
      />
...
> Note: confirm `/mark` supports a `?campaign=<id>` deep link that opens or creates a campaign-scoped thread. If not, change "Open in Mark" to link to the newest thread from `getCampaignThreads`, or to `/mark` plainly — and capture wiring a campaign deep-link as a follow-up. `CampaignDispatch` import path: match the real type from `@/lib/dispatch/read-model`.

- [ ] **Step 2: Delete now-dead tab components if unreferenced**

After the rewrite, these are no longer imported: `approvals-tab.tsx`, `creative-tab.tsx`, `performance-tab.tsx`, `reasoning-tab.tsx`, `audience-leads-tab.tsx`, `campaign-media-board.tsx`, `sticky-decision-bar.tsx`, `campaign-triage-strip.tsx`, `momentum-strip.tsx`, `mark-conversation.tsx`. Run a grep to confirm zero references, then delete. Keep `campaign-economics-panel.tsx` (still mounted by `page.tsx`).

```bash
grep -rl "creative-tab\|approvals-tab\|reasoning-tab\|sticky-decision-bar\|mark-conversation\|campaign-triage-strip\|momentum-strip" src/app/campaigns
```
  ...
  return (
    <div className="flex flex-col gap-3">
      <form action={askMarkToBuildCampaignAction} className={cx(theme.surface.panel, "flex flex-col gap-2 rounded-xl p-4")}>
        <div className="flex items-center gap-2"><MarkAvatar size={20} /><span className="text-sm font-semibold text-[var(--text-primary)]">Ask Mark to build it</span></div>
        ...
        <button type="submit" className="self-start rounded-lg bg-[var(--accent)] px-3 py-1.5 text-[12px] font-semibold text-[var(--on-accent)]">Build with Mark</button>
      </form>
      ...
      <button onClick={() => setMode("manual")} className={cx(theme.surface.inset, "rounded-xl border p-4 text-left")}>
        ...
        <div className="text-[12px] text-[var(--text-muted)]">Create it by hand, then hand to Mark anytime.</div>
      </button>
    </div>
  );
}
...
git commit -m "feat(campaigns): who-drives creation chooser (ask Mark / manual)"
```
...
- `/campaigns` — ask-Mark bar, filters, status cards with driver avatars + gold "needs you".
- `/campaigns/<id>` — Overview: At a glance, package with inline Approve/Decline, What's live, Audience, Work lane, Threads, History; "Open in Mark" + Launch in the header.
...
- **Spec coverage:**
  - List page cards + ask-Mark bar + needs-you filter + new-campaign chooser → Tasks 5–8, 11. ✓
  ...
  - Board-as-spine: campaign tasks via `campaign_id`, driver avatars, "+ Hand to Mark", "Open in board" → Tasks 3–5, 9. ✓
  - Mark handoff: ask-to-build creates shell + linked conversation + queued task + redirect to `/mark`; "Open in Mark" → Tasks 5, 10. ✓
...
- **Deferred to later plans (per spec):** Board view toggle on the campaign page, `/board?campaign=` filtering UI + human-owned tasks migration (Phase 2), live Mark dock (Phase 3).
```
