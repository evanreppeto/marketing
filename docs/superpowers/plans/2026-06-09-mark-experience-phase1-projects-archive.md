# Mark Experience — Phase 1 (Projects + Archive + Polish) Implementation Plan
...
**Goal:** Make the Mark chat sidebar feel professional — group conversations into projects, archive/unarchive chats, and fix the sparse thread layout — with no dependency on Mark's runtime.

**Architecture:** Add a `mark_projects` table + nullable `mark_conversations.project_id`; extend `src/lib/mark-chat/persistence.ts` with project + archive helpers; add server actions in `src/app/mark/actions.ts`; group the sidebar and add an archived view in `src/app/mark/_components/`. Live step-by-step activity is a SEPARATE Phase 2 plan.
...
> **⚠️ BUILD-COORDINATION CAUTION:** `src/app/mark/**` and `src/lib/mark-chat/persistence.ts` are under **active parallel development**. The UI code below is written against the files as of 2026-06-09 — **re-read each file immediately before editing and reconcile** (the data/persistence/action layers are stable and low-conflict; the component edits are the churny part). Make small, focused commits.

---

## File map
- DB: `mark_projects` table + `mark_conversations.project_id` (MCP `apply_migration`, name `mark_projects`).
- Modify `src/lib/mark-chat/persistence.ts` — project type/CRUD, archive list/unarchive, `project_id` on conversation.
- Create `src/lib/mark-chat/persistence.projects.test.ts` — persistence tests (mock client).
- Modify `src/app/mark/actions.ts` — project + move + unarchive server actions.
- Modify `src/app/mark/page.tsx` — load projects (+ archived when `?archived=1`) and pass down.
- Modify `src/app/mark/_components/thread-sidebar.tsx` — grouped projects, archive section, controls.
- Modify `src/app/mark/_components/mark-chat.tsx` — thread props for projects/archived.
- Modify `src/app/mark/_components/message-list.tsx` — layout polish (column constraint).

---

## Task 1: Database — projects table + conversation link

- [ ] **Step 1: Apply migration** via Supabase MCP `apply_migration` (project `fpjvgqrfqncnudqeudee`, name `mark_projects`):
...
create index if not exists mark_conversations_project_idx on public.mark_conversations(project_id);
```

- [ ] **Step 2: Verify** with `execute_sql`: `select column_name from information_schema.columns where table_name='mark_conversations' and column_name='project_id';` returns one row, and `mark_projects` exists.

## Task 2: Persistence — surface `project_id` on conversations

**File:** `src/lib/mark-chat/persistence.ts`
...
- [ ] **Step 3: Verify** `pnpm vitest run src/lib/mark-chat` (existing tests, if any) + `pnpm lint` stay green.

## Task 3: Persistence — projects CRUD + archive helpers (TDD)

**Files:** `src/lib/mark-chat/persistence.ts`; Test `src/lib/mark-chat/persistence.projects.test.ts`
...
describe("mark projects/archive persistence", () => {
  it("createProject inserts a project for the operator and returns it", async () => {
    const supabase = createSupabaseQueryMock({
      mark_projects: { data: { id: "p1", operator: "Evan", name: "Storm Q3", status: undefined }, error: null },
    });
...
- [ ] **Step 2: Run to verify fail** — `pnpm vitest run src/lib/mark-chat/persistence.projects.test.ts` → cannot find exports.

- [ ] **Step 3: Implement** — add to `src/lib/mark-chat/persistence.ts`:
...
- [ ] **Step 4: Run to verify pass** — `pnpm vitest run src/lib/mark-chat/persistence.projects.test.ts` (3 passed). Adjust the mock-call destructuring only if the helper's tuple shape differs.

- [ ] **Step 5: Commit** — `git add src/lib/mark-chat/persistence.ts src/lib/mark-chat/persistence.projects.test.ts && git commit -m "feat(mark-chat): projects + archive persistence helpers"`

## Task 4: Server actions

**File:** `src/app/mark/actions.ts` (re-read first; `archiveThreadAction` already exists — mirror its shape)
...
- [ ] **Step 4: Commit** — `git add src/app/mark/actions.ts && git commit -m "feat(mark-chat): project/move/unarchive server actions"`

## Task 5: Load projects + archived in the page

**File:** `src/app/mark/page.tsx` (re-read first)
...
**File:** `src/app/mark/_components/thread-sidebar.tsx` (re-read first; props now include `projects: MarkProject[]`, `archived: MarkConversation[]`, `showArchived: boolean`)

- [ ] **Step 1:** Group conversations by `projectId`. Render each project as a labeled group (project name → its chats), then an "Chats" group for `projectId === null`. Reuse the existing chat-link markup. Add a **New project** control (a small form posting `createProjectAction`) and, per chat, a **move/archive** affordance (hover menu posting `moveConversationAction` / `archiveThreadAction`). Add an **Archived** toggle linking to `/mark?archived=1`; when `showArchived`, render the `archived` list with an **Unarchive** control (`unarchiveThreadAction`). Keep the existing visual tokens/classes.
...
**File:** `src/app/mark/_components/mark-chat.tsx` (re-read first)
...
- **Schema safety:** `mark_projects` required cols = operator, name (both provided); `project_id` nullable on conversations; reuses existing `set_updated_at`.
