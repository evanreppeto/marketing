# Shared Human + Arc Ticket Detail Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Linear-style task detail page for the Growth Engine board where humans and Arc share editable tickets, latest outputs, activity, and approval guardrails.

**Architecture:** Add a small persistence layer around `agent_tasks` for human/Arc ownership fields and task events. Extend `src/lib/agent-operations/read-model.ts` to produce ticket-detail data, then build focused client components for editable header/properties while preserving the existing inputs/outputs/log panels.
...
This plan implements one subsystem: the shared human + Arc ticket detail and the board data it depends on. It does not implement a full Linear clone, realtime multi-user collaboration, a board drawer, publishing/sending/launching, or a broader campaign redesign.
...
- Create `src/app/agent-operations/tasks/[taskId]/ticket-activity-timeline.tsx`
  - Unified human/Arc/system/approval timeline.
...
Expected: either PASS, or failures only from unrelated in-flight Arc auth work. If it fails, record the exact unrelated files before continuing.
...
it("maps task detail into a shared human and Arc ticket workspace", async () => {
  ...
  expect(detail.task).toMatchObject({
    owner: { kind: "human", label: "Evan" },
    driver: { kind: "agent", label: "Arc", agentId: "agent-1" },
    approverLabel: "Owner",
    description: "Build the first partner-facing draft and keep outbound locked.",
  });
  ...
  expect(detail.timeline.map((event) => event.source)).toContain("Arc");
...
function mapActor(kind: string | null, label: string | null) {
  ...
  return { kind: normalizedKind, label: label?.trim() || (normalizedKind === "agent" ? "Arc" : "Operator") };
}
...
function mapEventSource(row: Pick<AgentTaskEventRow, "actor_kind" | "actor_label">): "Human" | "Arc" | "System" | "Approval" {
  ...
  return (
    <header className="space-y-4">
      ...
      <textarea
        ...
        placeholder="Add the human-readable work brief Arc and the team should follow."
        value={brief}
      />
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button className={buttonClasses({ variant: "ghost", size: "sm" })} onClick={submitInstruction} type="button">Ask Arc to continue</button>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Add an instruction for Arc..."
          value={instruction}
        />
...
export function TicketLatestOutput({ output, taskId }: { output: Output | null; taskId: string }) {
  if (!output) {
    return <section className="rounded-xl border border-dashed border-[var(--border-hairline)] bg-[var(--surface-inset)] p-4 text-sm text-[var(--text-muted)]">No output yet. When Arc creates work, the latest useful output appears here.</section>;
  }
  return (
    ...
    <ol className="space-y-3">
      {events.map((event) => (
        <li className="grid grid-cols-[28px_1fr] gap-3" key={event.id}>
          <div className={`mt-1 flex h-7 w-7 items-center justify-center rounded-full border text-[10px] font-semibold ${event.source === "Arc" ? "border-[var(--accent-border)] text-[var(--accent)]" : "border-[var(--border-hairline)] text-[var(--text-muted)]"}`}>
            {event.source === "Arc" ? "M" : event.source.slice(0, 1)}
          </div>
...
Expected: PASS, except for unrelated existing Arc auth work if still present. Fix only errors caused by these files.
...
await supabase.from("agent_task_events").insert({
  ...
  title: "Task created",
  body: objective,
  metadata: { source: "board_create", driver: "Arc" },
});
```
...
Keep `EntityAvatar owner={{ kind: "agent" }}` for Arc-driven cards, but switch to human owner when `driverIsMark` is false:
...
```powershell
pnpm test src/lib/agent-operations/read-model.test.ts 'src/app/agent-operations/tasks/[taskId]/actions.test.ts' src/lib/arc-api/__tests__/tasks.test.ts
```
...
Expected: PASS. If it fails because of pre-existing Arc auth files, capture exact errors and do not mix that fix into this slice without user approval.
...
- Activity timeline shows human, Arc, system, and approval events where data exists.
