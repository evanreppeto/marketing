# Campaign Slim-Intel + Mark Drawer Redesign — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the campaign workspace's fixed 360px right rail with a full-width content area, a slim collapsible "decision context" bar, and a larger slide-over "Chat with Mark" drawer (openable campaign-wide and pre-targeted per asset), leaving a disabled hook for future Gemini/Veo/Nano Banana generation.
...
**Spec:** `docs/superpowers/specs/2026-06-02-campaign-mark-drawer-intel-redesign-design.md`
...
- **Create** `src/app/campaigns/_components/mark-drawer.tsx` — slide-over "Chat with Mark" panel: disabled generate hook, enlarged revise form (`requestRevisionAction`), and a read-only event activity thread. Owns the Mark-interaction responsibility. Takes discrete props (no shared context object).
- **Modify** `src/app/campaigns/_components/campaign-workspace.tsx` — drop the 2-col grid, add drawer open state, mount `DecisionContextBar` + `MarkDrawer`, add the "Chat with Mark" trigger in the tab row, repoint `pickAsset` to open the drawer.
...
- **Delete** `src/app/campaigns/_components/mark-rail.tsx` — superseded.
...
Reuses `IntelligencePanel` / `IntelligencePanelModel` from `@/app/_components/intelligence-panel`, `StatusPill` from `@/app/_components/page-header`. The intel model object is the exact one `mark-rail.tsx:36-53` builds today.
...
**Files:**
- Create: `src/app/campaigns/_components/mark-drawer.tsx`

Reuses `requestRevisionAction` from `../actions`, `Button`/`buttonClasses` from `@/app/_components/page-header`, and the `CampaignWorkspaceEvent` type from `@/lib/campaigns/read-model` for the activity thread. This file owns what `mark-rail.tsx` did, minus the intel panel (now in Task 1) and as a drawer rather than a rail.
...
export function MarkDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Chat with Mark">
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-[var(--border-hairline)] bg-[var(--surface-inset)] px-5 py-4">
          <div>
            <div className="signal-eyebrow">Mark</div>
            <h2 className="mt-1 text-lg font-black tracking-[-0.02em] text-[var(--text-primary)]">Chat with Mark</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          >
            <span>
              <span className="block text-sm font-bold text-[var(--text-primary)]">Generate creative with Mark ▸</span>
...
```bash
git add src/app/campaigns/_components/mark-drawer.tsx
git commit -m "feat(campaigns): slide-over Mark drawer with enlarged revise form + activity"
```
...
import { MarkDrawer } from "./mark-drawer";
```
...
- [ ] **Step 4: Make the content full-width and add the Mark trigger in the tab row**
...
Then, in the tablist row, add a "Chat with Mark" trigger aligned to the right. Change the `role="tablist"` container's className to push the button to the end and add the button as the last child inside that row. Replace:
...
git commit -m "feat(campaigns): full-width workspace with intel bar + Mark drawer trigger"
```
...
**Files:**
- Delete: `src/app/campaigns/_components/mark-rail.tsx`

- [ ] **Step 1: Confirm nothing else imports it**

Run: `git grep -n "mark-rail\|MarkRail" -- src/`
Expected: No matches (Task 3 removed the only import).

- [ ] **Step 2: Delete the file**

```bash
git rm src/app/campaigns/_components/mark-rail.tsx
```
...
Run: `pnpm dev`, open a campaign workspace at `/campaigns/<id>` (seed with `pnpm seed:hermes-demo` if no live campaigns), and confirm:
...
- "Chat with Mark" (right of the tabs) opens the slide-over drawer; the scrim, the Close button, and the `Esc` key all dismiss it; body scroll is locked while open.
- On the Creative tab, an asset card's "Ask Mark to revise" opens the drawer with that asset pre-selected in the Target asset dropdown.
...
- The "Generate creative with Mark ▸" row is visibly present but disabled.
...
```bash
git add -A
git commit -m "test(campaigns): verify slim-intel + Mark drawer redesign"
```
...
- **Spec coverage:** rail removal + full-width (Task 3); slim intel bar + disclosure (Task 1, mounted Task 3); larger Mark drawer with per-asset + campaign-wide open (Tasks 2–3); activity thread (Task 2 via `detail.events`); disabled Gemini/Veo/Nano hook (Task 2); `mark-rail.tsx` removal (Task 4); `IntelligencePanel`/`requestRevisionAction`/auth unchanged (reused verbatim). Follow-up generation work is explicitly out of scope per spec.
- **No new design tokens or backend changes** — every class uses existing CSS vars; the intel model in Task 1 is copied verbatim from `mark-rail.tsx:36-53` to preserve identical content.
