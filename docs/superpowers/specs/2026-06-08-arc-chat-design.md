# Design: "Talk to Arc" — Conversational Control Surface

Date: 2026-06-08
Status: Approved (pending spec review)
Route: `/arc` (becomes the primary "Arc" nav surface)

## Summary

A ChatGPT-style page for talking to **Arc** (the Arc agent). The operator
holds multi-thread conversations, @-mentions specific records (campaigns, leads,
CRM objects, personas, vault notes) to ground the discussion, and Arc replies
asynchronously. The page is the new primary "Arc" surface; the existing
...
bearer-gated callback API for Arc's replies, and **outbound stays locked** —
the chat never sends anything externally.

## Decisions (from brainstorm)

- **Arc's brain = the real Arc agent**, not an in-app LLM. This app holds no
  model key; Arc is the brain and integrates via the existing bearer-gated
  API pattern.
- **Connection = async queue + callback.** Operator message is persisted and
  enqueued as an `agent_task`. Arc processes it and POSTs its reply back to a
  new bearer-gated endpoint `POST /api/v1/arc/messages`. No outbound/streaming
  dependency from this app.
- **Multi-thread** with a left history sidebar (ChatGPT-style), not a single log.
- **Chat is the primary Arc page** at `/arc`; nav "Arc" repointed there; the
...
- No in-app LLM / Anthropic / OpenAI integration. Arc is the brain.
...
- No outbound sending, publishing, launching, or spend from the chat. Any action
  Arc proposes routes through the existing approval pipeline; the chat only
  links to it.
...
`src/domain/arc-chat.ts` (pure) → `src/lib/arc-chat/` (I/O, persistence,
mention search) → `src/app/arc/` (server components + colocated `_components/`,
`actions.ts`) + `src/app/api/v1/arc/messages/route.ts` (callback).
...
`arc_messages`
- `id` uuid pk
- `conversation_id` uuid fk → arc_conversations(id) on delete cascade
- `role` text check in ('operator','arc','system')
- `body` text
- `status` text check in ('sent','pending','complete','failed') default 'sent'
  - operator messages insert as `sent`; the Arc placeholder inserts as
    `pending` and flips to `complete` (or `failed`) on callback.
- `agent_task_id` uuid null (links a Arc `pending` message to the queued task)
- `mentions` jsonb default '[]' — `[{type,id,label,href}]`
- `created_at` timestamptz default now()
- `metadata` jsonb default '{}'

Indexes: `arc_messages(conversation_id, created_at)`,
`arc_conversations(operator, last_message_at desc)`.
...
### Domain logic — `src/domain/arc-chat.ts` (unit-tested in `__tests__`)
...
### Persistence + glue — `src/lib/arc-chat/`
...
- `enqueue.ts` — wraps/extends `sendArcDirective` (`src/lib/campaigns/
  arc-conversation.ts`) to create the `agent_task` (`task_type:
  "arc_chat_message"`, `metadata: { conversation_id, message_id, mentions,
...
- `mention-context.ts` — `resolveMentionSnapshots(mentions)` → minimal record
  summaries attached to the task so Arc has grounded context.

All persistence guarded by `isSupabaseAdminConfigured()`; degrade gracefully.

### Server actions — `src/app/arc/actions.ts`

All `"use server"`, gated by `requireOperator()` + `isSupabaseAdminConfigured()`,
returning `{ ok, message, ... } | null` and calling `revalidatePath("/arc")`.
...
- `sendMarkMessage(conversationId | null, formData)`:
  ...
  5. Insert `pending` Arc message bound to `agent_task_id`.
...
### Callback API — `src/app/api/v1/arc/messages/route.ts`

- `POST`, bearer-gated `checkBearerToken(request, "ARC_AGENT_API_TOKEN")`.
- Body: `{ conversationId, replyTo (agentTaskId or operator messageId), body,
  status?: "complete" | "failed", metadata? }`.
- Looks up the `pending` Arc message by `agent_task_id` (or conversation +
...
- `message-list.tsx` / `message-row.tsx` — message rows (avatar + role label +
  ...
  pills, JetBrains-mono timestamps. A `arc` message with reference metadata
  renders a small reference card linking to the approval/campaign surface.
- `thinking-indicator.tsx` — CSS-only pulsing tri-dot + "Arc is thinking…"
  (`status-breathe`, transform/opacity only, `prefers-reduced-motion` honored).
  Shown whenever the latest Arc message is `pending`. Optional progress label
...
`src/app/_data/growth-engine.ts`: repoint the "Arc" `navItems` entry from
`/agent-operations` to `/arc`. The chat header carries an "Operations ▸" link
...
- Send failure / task enqueue failure → inline result banner; Arc message can be
  marked `failed` with a retry affordance.
- Mention search with no results → quiet "No matches" in the popover.

## Testing / Verification

- `src/domain/__tests__/arc-chat.test.ts`: title derivation, input validation,
...
1. Migration + `src/domain/arc-chat.ts` + tests + `src/lib/arc-chat/`
   persistence/repos.
...
- Message ordering: composer disabled while pending keeps the per-thread queue
  ordered; Arc replies are matched by `agent_task_id`.
- `agent_task` reuse: `task_type: "arc_chat_message"` is new; the
  agent-operations dashboard read-models may surface these tasks — acceptable
  (they're legitimate Arc work), but verify they don't break existing filters.
- The Arc agent must implement the new callback contract; document it
  alongside `/api/v1/arc/runs` (token doc).
