# Campaigns × Arc — Workspace Redesign
...
marketing with Arc as a teammate. Two things must be true at all times:
...
2. **User and Arc are intertwined.** Arc can build a campaign, the user can
   build one manually, either can hand it to the other at any point, and *most
   of the work happens in the Arc chat* — including revisions, which spin up a
...
The board, the campaign, and the Arc chat are **not separate features** — they
...
```
A board card  ⇄  a Arc thread  ⇄  a campaign
...
- **Arc chat** = where work actually happens, always present inside a campaign.
...
|-------|-------------|-------|---------|---------------|
| Building | **Arc building** | blue | Arc has active/queued work, nothing waiting on the human | `Drafting` |
...
"Who's driving" (Arc vs. you) is reinforced by an explicit **handoff control**,
...
**Header:** title "Campaigns", one-line plain description ("Everything Arc is
...
4. **Arc is working on these** (blue) — `Building`
5. **Finished** (collapsed) — `Done`

**Each row** shows: campaign icon, name, one-line "For {persona} · Goal:
{objective}", a **"who's driving" tag** (`Needs you` / `Arc building` / `Live` /
...
- Two "what's working" bars: **which campaigns book work** and **which audiences
  respond** — feeds the operator's gut *and* Arc's learning loop (he cites these
...
- Plain-English **what / who / why**: "3 pieces ready · 2 IG ads + 1 email" /
  "Reaches 1,240 property managers" / "Why Arc built it: May storms spiked
  water-damage demand."
...
- 🗣️ **Tell Arc what you need** — describe it in plain words; Arc creates a
  shell campaign and drafts the first pieces. Opens a new Arc thread under the
  new campaign.
- ✏️ **Set it up myself** — the existing `campaign-create-form.tsx` fields
  ...
  offer **"Hand to Arc to build"**.

**Handoff (give-to-Arc / take-it-back) is available anytime** via an explicit
control in the campaign header, surfaced through the same "who's driving"
language. Handing to Arc queues an `agent_task` for the campaign (the mechanism
`sendArcDirective` already uses); taking it back pauses Arc's queued work and
...
content fills the main column; **Arc lives in a side pane** to build & revise.

**Top of page (always visible):**
- Breadcrumb back to Campaigns, campaign name.
- **Progress tracker (new):** a simple stepper — *Idea → Arc built it → Your
...
- **Handoff control:** "Hand to Arc" / "Take it back".
...
"Reasoning / Talk to Arc" is **no longer a tab** — Arc is the always-present
side pane.

**Side pane — Arc (the heart of the redesign):**
- Hosts a **Arc thread** for this campaign with a **thread switcher** (▾).
...
- **"Arc suggests" (new):** Arc proposes the next step in plain words ("People
...
Today there are **two** Arc mechanisms touching campaigns:
1. `campaigns/_components/arc-conversation.tsx` — a single per-campaign directive
   conversation (queues `agent_tasks`, shown in the "reasoning" tab).
2. `arc-chat` conversations (`src/lib/arc-chat/`) — the rich `/arc` threads
...
**Decision:** the campaign's Arc side pane is built on the **`arc-chat`
...
and consistent with the standalone `/arc` surface — one chat system, two entry
points (global `/arc` and docked-in-campaign). The older single
`arc-conversation` directive view is retired (its `agent_task` queuing behavior
...
- **Campaign → board:** when Arc is building or something's waiting on you, a
  card already exists as an `agent_task`; surface it on the board with a link
  back to its campaign. Add a **"Send to Arc's board"** action on a campaign /
  draft to park a request for later.
- **Board → campaign:** tapping a card opens its campaign with the right Arc
...
- **Shared columns:** the board's columns relabel to the shared vocabulary —
  **To do (Queued) · Arc working (Running) · Needs you (Blocked) · Done
...
- **Mobile:** the split workspace is desktop-only. On a phone, the campaign
  content is full-width and Arc becomes a **"Talk to Arc" bottom sheet**.
- **First-run / empty state:** instead of a blank page, a warm "Let's make your
  first campaign" with Arc.
- **"What changed since you were last here":** a small "2 new drafts from Arc"
...
- **Design system:** the campaigns surface follows `DESIGN.md` (Command
  ...
  exception: `/arc` itself is a deliberate "alive" visual zone; the docked Arc
  pane in a campaign should feel consistent with `/arc` while the surrounding
  campaign chrome stays calm.

## What we reuse vs. build

**Reuse (most of it exists):** `agent_tasks` backbone, `arc-chat` conversations
...
`launch.ts`, `decideCampaignDraftAction`, `sendArcDirective`, board read-model,
...
attribution/economics data); progress-tracker stepper; docked Arc pane built on threads +
thread switcher; "Arc suggests" surface; go-live confirm dialog; Preview
...
- **Phase 2 — Detail restructure.** Split workspace, plain-language sections
  (merge Decision log + Audit), progress tracker, **consolidate Arc pane onto
  the threads system** with the thread switcher.
- **Phase 3 — Trust, proactivity & analytics.** True-to-life Preview section,
  go-live confirm, "Arc suggests," and the portfolio analytics strip.
...
2. **Thread auto-naming** — when a revision (or `+ New thread`) spawns a thread,
   **Arc auto-names it from the request** ("Revise the email"), editable by the
...
3. **Retiring the old reasoning tab** — during Phase 2, confirm nothing else
   ...
   `?tab=reasoning` to the docked Arc pane if any are found.
