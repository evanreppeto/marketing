# Shared Human + Mark Ticket Detail - Design
...
The Kanban board is meant to be shared operating space for humans and Mark, but the current click-through ticket page feels like a sparse read-only record. It shows a title, tabs, counts, and metadata, yet it does not work like an operational issue detail:
...
The next version should keep the app's purpose: a shared work board where humans and Mark coordinate restoration-specific marketing work, with outbound actions locked behind human approval.
...
- Mark/Hermes can be the active driver.
...
- Governance is explicit: Mark can prepare work, but cannot publish, launch, or bypass approval.

## Ownership Model

Use **co-ownership** instead of forcing a ticket to be either human-owned or Mark-owned.

- **Owner:** the human accountable for the ticket. This person can edit, reassign, stop, revise, and approve work.
- **Driver:** the current actor doing the next work. This can be Mark or a human.
- **Approver:** the human or approval role required before outbound or final assets can be used.

This keeps the board honest. A task can say "Mark is driving this" while still making it clear which human owns the outcome.
...
- `Ask Mark to continue` when Mark is available.
- `Request revision` when an output or approval item exists.
- `Assign to human` or `Assign to Mark`.
...
- Human instructions.
- Mark's output target.
- Acceptance criteria.
- Any guardrails that apply.

Acceptance criteria are lightweight checklist items. Humans can edit them. Mark may report progress against them, but completion of approval-sensitive work still requires human approval.
...
- Mark claimed/started/blocked/completed events.
- Mark outputs.
- Status changes.
- Approval events.
- System or audit events.

The activity tab can add filters later, but the first version can show a unified timeline with clear source labels: `Human`, `Mark`, `System`, `Approval`.
...
- Mark can update task state through the agent API where allowed.
...
- `driver_agent_id` when Mark/Hermes is the driver.
...
- `agent_task_events` as a small activity table for human comments, human edits, instructions to Mark, and system-visible task changes that are not already represented by outputs/logs/approval rows.
...
- Cannot publish, launch, send, export, or mark approval-sensitive work as externally usable.
...
- Use Mark's avatar for agent-driven activity and a human avatar/initials for human edits.
...
7. Verify that Mark/Hermes API behavior and approval guardrails still pass.
...
- existing Hermes task route tests
...
- Mark output appears pinned on the overview.
