# Arc Chat — Premium Experience Pass (Design)

**Date:** 2026-06-09
**Status:** Approved (design phase)
**Scope owner:** Arc chat surface (`src/app/arc/**`, `src/lib/arc-chat/**`) + Arc-worker contract docs

## Goal

Make the Arc chat feel premium and alive — fixing the barren "thinking" state, the blank
...
app-side and **degrades gracefully**: new affordances light up as Arc's worker sends the
...
- No **permissions layer** in the app: the mode picker *sends* the stance; Arc's worker enforces it.
...
- No change to the webhook **transport** (`/api/v1/arc/messages/**`); we only add fields to existing payloads.

## Architecture & the app/worker split

Repo convention holds: `src/domain/` (pure) → `src/lib/arc-chat/` (I/O) → `src/app/arc/`
(server page + colocated `_components/`). Server actions stay gated by `requireOperator()` +
`isSupabaseAdminConfigured()` + `revalidatePath("/arc")`.
...
**Delivered as worker contract docs (you wire on Arc's Mac worker):**
1. Emit steps via the existing `POST /api/v1/arc/messages/{agentTaskId}/steps` → lights up the waiting state.
...
All on the existing `arc_messages.metadata` jsonb and `agent_tasks.metadata` jsonb — **no
new tables**.

### `MarkActionCard` (`src/lib/arc-chat/persistence.ts`)
...
in `@/domain` (`parseArcMode`). `enqueueMarkChatTask` writes it into the `agent_tasks`
...
## Components (`src/app/arc/_components/`)
...
- **Toolbar:** add **Regenerate** (non-failed Arc replies → `onRegenerate`) and **👍/👎**
...
### `empty-state.tsx` + `arc-chat.tsx` (shell)

- Empty state becomes **input-first**: centered Arc emblem, `What can Arc help with?`, a
  restrained capability line ("Arc can find leads · draft campaigns · reference your records
...
## Backend (`src/app/arc/actions.ts`, `src/lib/arc-chat/**`)
...
- `regenerateMarkReplyAction(conversationId, markMessageId)` — find the operator message
  immediately preceding the given Arc reply, enqueue a fresh task + pending bubble for it
...
A new doc `docs/arc-worker-contract-premium.md` (and update the existing
`arc-chat-responder` skill) specifying:
1. **Steps:** `POST /api/v1/arc/messages/{agentTaskId}/steps {label, status}` before/after each action (endpoint exists).
...
- **Pure/unit (vitest):** `parseActions` (valid/partial/garbage), `parseArcMode` (valid/default), slash-command registry resolution, `sameMessages` updated to include `actions`. Follow existing `persistence.test.ts`/`steps.test.ts` patterns.
...
- Persisted feedback **analytics** to tune Arc (currently just stored on the message).
