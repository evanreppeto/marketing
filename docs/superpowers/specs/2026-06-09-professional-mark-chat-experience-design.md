# Professional Mark Chat Experience — Design
...
Make the app's **Mark** tab feel like a polished, professional assistant experience:
1. **Live step-by-step activity** — while a reply is pending, show what Mark is *actually* doing as a live checklist that resolves into his answer (not a generic "thinking…").
...
- Live activity = **real step-by-step feed** (Mark reports each step), not a generic animation.
- Projects v1 = **grouping only** (no per-project custom instructions yet — future).
- Archive = **UI on the existing backend** (`archiveConversation`, `status='archived'` already in `src/lib/mark-chat/persistence.ts`).

## Architecture / conventions
Follows the app's existing layering: `src/domain` (pure) → `src/lib/mark-chat` (I/O) →
`src/app/mark` (UI) + `src/app/api/v1/hermes` (agent API). Reuse existing patterns:
...
**Build-coordination caution:** `src/app/mark/**` and `src/lib/mark-chat/**` are under
...
**Data model.** A pending Mark message carries an ordered list of steps in its existing
`metadata` jsonb: `metadata.steps: [{ label: string, status: "running"|"done", at: string }]`.
No new table. `mark_messages.metadata` and `MESSAGE_COLUMNS` already include `metadata`,
so steps flow through the existing message read path.

**New endpoint.** `POST /api/v1/hermes/messages/{agentTaskId}/steps` (bearer
`HERMES_AGENT_API_TOKEN`, mirrors the other hermes routes). Body:
`{ "label": string, "status"?: "running"|"done" }` (default `running`). Behavior:
find the pending Mark message by `agentTaskId` (reuse `findPendingMessageByTask`), append
...
`src/lib/mark-chat/persistence.ts` (read row → merge step into `metadata.steps` → update).
Extend `toMessage` to parse `metadata.steps` into `MarkMessage.steps` (typed
`MarkStep[]`).

**Read/UI.** `MarkMessage` gains `steps: MarkStep[]`. In `message-list.tsx`, when a Mark
message is `pending`, render an **ActivityTimeline** (each step: `✓` done / `⟳` running,
label, subtle stagger) instead of the bare "Waiting for Mark…". When the message turns
`complete`, render the final body with the step trace collapsed into a small
"Show what Mark did" disclosure. Existing 2.5s thread polling drives the live updates —
no new client machinery.

**Mark's side (his worker).** Update the `mark-chat-responder` skill: before each
...
local migrations dir): table `mark_projects (id uuid pk, operator text not null,
...
`project_id uuid references mark_projects(id) on delete set null` to `mark_conversations`.

**Persistence** (`src/lib/mark-chat/persistence.ts`): `createProject`, `listProjects(operator)`,
...
**Server actions** (`src/app/mark/actions.ts`, gated by `requireOperator` +
...
- Fix the thread layout: constrain messages to the centered column so short operator
  ...
  look). Verify operator bubble + Mark message align within the same column.
...
## Phasing
- **Phase 1 (build now, app-only):** Projects (B) + Archive (C) + Polish (D). No Mark
...
- **Phase 2 (cross-system):** Live step-by-step activity (A) — the steps endpoint + UI
  timeline + Mark's worker emitting steps. Can be promoted ahead of Phase 1 if desired,
  but it requires Mark-side coordination.

## Testing
- **Domain/persistence:** unit-test `appendMarkStep` step-merge logic and project
  create/list/move/assign with `createSupabaseQueryMock` (mirror existing mark-chat tests).
...
sharing of projects; reordering/pinning; analytics on Mark's steps.
