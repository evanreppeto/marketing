# Mark Campaign Studio — Design Spec
...
Today a Mark reply renders one draft thinly: an inline copy-card plus a loose image, and a right-side "Work canvas" that only shows the single latest draft. Per the new `CLAUDE.md` charter, Mark is BSR's lead-marketing operator that produces **campaign packages** (many assets) and must surface **evidence, approval state, media, and provenance** obviously. The chat needs to render whole campaigns, and the side panel needs to become a persistent home for everything Mark generates.

## Vision

Reframe Mark from "a chat that drafts one thing" into a **campaign studio**. The chat is the conversation + the moment Mark produces something; the right panel ("**Studio**") is the persistent workspace holding every asset/media item in flight for the thread's campaign. Everything stays **outbound-locked** until a human approves.
...
- **Project** (`MarkProject`) — an **initiative folder** that groups chat threads (and, conceptually, can span multiple campaigns). Organization only; no assets, no approval, no outbound. Lives in Mark chat.
...
`src/domain/mark-chat.ts`. All new fields optional; existing data and parsers keep working.
...
New `src/app/mark/_components/campaign-deck.tsx`.

- **Trigger:** a Mark message with **≥2 draft action cards** renders a `CampaignDeck` instead of stacked `ActionCard`s. A single draft keeps today's `ActionCard` (with folded image).
...
`src/app/mark/_components/work-canvas.tsx` → reframed as **Studio**, tabbed. Keep the docked/drawer + toggle behavior already shipped.

- **Tab: Now** — current behavior. Building timeline while pending; otherwise the latest deliverable (image hero + copy + approve, with the read-only fallback for preview). Default tab while Mark is actively working.
...
- Real multi-asset replies require Mark's worker to post messages whose `metadata.actions` is an array of asset cards with the new fields — a **Mark contract** deliverable (mirror the steps contract in the Phase-2 plan). Documented, not built here.
...
- **No emojis** in UI (SVG icons only); follow `DESIGN.md` tokens. `/mark` may use richer "alive" visuals (memory: mark-chat visual exception) but stays on-palette.
...
- **Modify:** `domain/mark-chat.ts` (types + parsers), `work-canvas.tsx` (→ Studio tabs + library), `message-list.tsx` (deck trigger), `action-card.tsx` (provenance/status chips), `_data/demo.ts`.

## Out of scope (Phase 2+)

Audience tab; live merge of linked-campaign assets from Supabase; variant A/B/C diffing UI; metrics/performance cards; drag-reorder; Higgsfield (stays flag-off until subscription confirmed); the Mark worker contract for emitting multi-asset packages; `campaigns.project_id` migration.
