# Arc Chat — Polish & Completion Design

**Date:** 2026-06-09
**Status:** Approved (design phase)
**Scope owner:** Arc chat surface (`src/app/arc/**`, `src/lib/arc-chat/**`)

## Goal

Turn the Arc chat tab into a professional, complete, maintainable marketing-assistant
...
Arc's product role: an assistant the operator talks to like an LLM, who can also *act*
in the app (find leads, add them to the CRM, generate campaigns). The UI must make those
actions legible — when Arc references or creates a record, the operator can click into it.

## Non-goals

- No change to the Arc webhook contract (`/api/v1/arc/messages/**`) or the
  Mac-side `arc-chat-responder` skill.
- No live AI in the app; outbound stays locked. Arc still replies via the existing
  enqueue → pending bubble → poll → complete flow.
- No full "action result cards" backend (deferred — see Follow-ups). We render richer
  result affordances from the data Arc *already* sends (`mentions`, `media`).
- Exactly **one** additive DB migration (`pinned_at`). Nothing else touches schema.

## Architecture & layering

Follows the repo convention: `src/lib/arc-chat/` (I/O, persistence) → `src/app/arc/`
(server page + colocated `_components/`). Server actions in `src/app/arc/actions.ts` stay
gated by `requireOperator()` + `isSupabaseAdminConfigured()` and `revalidatePath("/arc")`.

### Data layer changes (`src/lib/arc-chat/persistence.ts`)
...
- New helpers:
  ...
  - `deleteConversation(id)` — hard-deletes the conversation (messages cascade via FK; if
    no cascade exists, delete `arc_messages` for the conversation first).
...
`<ts>_arc_conversations_pinned_at.sql` — `alter table arc_conversations add column
...
All validate input, guard auth + Supabase, and `revalidatePath("/arc")`.

## Component design

Files under `src/app/arc/_components/`. Keep each focused; extract shared bits.

### New / extracted units

- `use-thread-poll.ts` (hook) — extracts the pending-reply polling loop currently inline in
  `arc-chat.tsx` (`sameMessages` + interval + safety cap). Inputs: `activeId`,
  `messages`, `setMessages`. Returns nothing; owns its own effect lifecycle. Makes
  `arc-chat.tsx` thin and the polling independently testable.
...
- **Hover toolbar** per message row (appears on hover/focus, keyboard reachable):
  - Arc replies: **Copy** (clipboard, with a 1.5s "Copied" confirm), and when it's the
...
- **Richer thinking state** (pending arc message):
  ...
  - When `steps.length === 0`: shimmer "Arc is thinking…" line (CSS gradient sweep on text,
...
- **Stop** button: optimistically removes the pending bubble, stops the poll, and calls
  `cancelReplyAction`. If Arc's reply still lands later (race), the next natural refresh
  shows it — acceptable.
- **Result affordances** (zero-backend slice of "action cards"): when a Arc reply carries
...
After this pass: `arc-chat.tsx` is a thin shell (no inline polling), the sidebar row logic
...
- Full **action-result cards** (Arc emits structured "created lead / drafted campaign"
  payloads → rich cards). Needs a Arc-contract change.
- **Saved prompts / prompt library** for repeated marketing tasks.
- **Message feedback** (👍/👎) persisted to tune Arc.
