# Mark Task Board (Kanban) + Scheduler — Design
...
A shared Kanban board at `/agent-operations` where the operator and the Hermes
agent (surfaced as **Mark**) both create and move task cards, backed by the
...
- A bearer-gated agent API at `/api/v1/hermes/tasks` with `claim / log /
...
| # | Decision | Choice |
|---|----------|--------|
| 1 | Board's primary job | **Shared workspace** — operator and Mark both create & move cards |
...
- `src/domain/hermes-tasks.ts` (extend) — add `canTransition(from, to)` and the
...
- Validity decided by pure `canTransition(from, to)` in `hermes-tasks.ts`
...
The feature must be sellable to anyone running a Hermes agent. Enforced
...
2. **No new hardcoded "Mark"/"Big Shoulders".** Agent identity, name, and
...
3. **Per-agent-token-ready API.** New agent-facing code resolves "which
   ...
   `HERMES_AGENT_API_TOKEN`, so each customer's agent authenticates into its own
   lane.
