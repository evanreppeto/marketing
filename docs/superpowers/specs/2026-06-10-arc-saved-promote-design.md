# Arc — Save & Promote ("Sandbox → Production")

**Date:** 2026-06-10
**Status:** Concept approved; spec pending user review
**Surfaces:** `src/app/arc/` (chat), `src/lib/arc-chat/`, a new Saved view, a chat-settings popover (attach project + campaign), ties into `/campaigns`

## Goal

Give the operator a lightweight middle stage between casual chat exploration and the
formal campaign approval pipeline: **star Arc's good outputs, keep experimenting on
...
Save the *creative* Arc generates (a media asset or an ad/draft action card) **and**
...
New migration `supabase/migrations/<ts>_arc_saved_items.sql`:

```sql
create table public.arc_saved_items (
  ...
  source_message_id uuid,          -- arc_messages.id the item came from (no hard FK; metadata-derived)
...
create index arc_saved_items_operator_idx on public.arc_saved_items (operator, created_at desc);
create index arc_saved_items_kind_idx on public.arc_saved_items (kind);
```
...
`arc_conversations` already has `project_id`. The conversation read-model
(`CONVERSATION_COLUMNS`, `toConversation`) gains `campaignId`, mirroring `projectId`.

## Units

### 1. Persistence — `src/lib/arc-chat/saved.ts` (new)

Pure I/O over `arc_saved_items`, guarded by `isSupabaseAdminConfigured()`:
...
### 2. Save action + star affordance — `src/app/arc/actions.ts`, `message-list.tsx`
...
- UI: a quiet ⭐ button on
  ...
  - each **Arc message** (the message action bar in `message-list.tsx`) as a
...
### 3. Saved view — `src/app/arc/saved/page.tsx` (new) + `_components/`

- New route `/arc/saved`, reachable from a **"Saved"** link in the thread sidebar
...
- **Continue in chat** opens the source conversation (`/arc?c=<sourceConversationId>`),
...
- Implemented by linking to `/arc?c=…&seed=…` or passing the seed via the existing
  per-thread draft mechanism (sessionStorage `arc:draft:*`). No backend change.

### 5. Promote → existing OR new campaign — `src/app/arc/actions.ts` + `src/lib/campaigns/`
...
Steps:
  ...
  5. Log a `campaign_events` `asset_generated` (actor=operator, detail='promoted from Arc saved').
  6. `markPromoted(savedItemId, { campaignId, assetId })`; `revalidatePath('/campaigns')` + `'/arc/saved'`.
...
### 6. Chat settings (gear) + campaign attachment — `arc-chat.tsx`, new `_components/chat-settings.tsx`, `actions.ts`, persistence
...
- **Header gear**: a settings button in the chat header (next to `MarkConnection` /
  ...
  - **Project** — the same project list; sets `arc_conversations.project_id`
  ...
  - **Campaign** — pick from the operator's campaigns (`getCampaignWorkspaceList()`);
    sets `arc_conversations.campaign_id`.
- **Persistence**: `assignConversationToCampaign(conversationId, campaignId | null)` in
  `src/lib/arc-chat/persistence.ts` (mirrors `assignConversationToProject`); read-model
  exposes `campaignId`.
- **Action**: `attachCampaignForm(formData)` (operator-gated, supabase-guarded) →
  `assignConversationToCampaign`; `revalidatePath('/arc')`.
...
- **Routing effect**: the attached `campaignId` is the Promote default (Unit 5). Arc's
  ...
  **follow-on** (the enqueue already carries conversation context; passing `campaignId`
  through `enqueueMarkChatTask` so Arc files outputs there is out of scope for Phase 1).
...
`/arc` (server) loads saved-source keys for star state → `MessageList`/`ActionCard`
render ⭐ → `saveMarkItemAction` inserts → `/arc/saved` lists items → **Promote** picks
...
- **Unit (vitest):** `saved.ts` (save/list/remove/find/arcPromoted) with the Supabase
...
- **Manual:** save a media item, a draft card, and a message as an angle; star
  reflects saved; `/arc/saved` lists all three groups; Promote → **existing** campaign
