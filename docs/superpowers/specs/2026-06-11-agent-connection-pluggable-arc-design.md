# Agent Connection — Pluggable Arc Design Spec

**Date:** 2026-06-11
**Status:** Approved direction. Sub-project 1 of the "others use it with their own Arc agent" goal.

## Problem

The app is a control plane for *an agent* (surfaced as **Arc**, run by **Arc**). It already talks to that agent through a clean two-way contract, but the connection is **single-tenant and env-only**: the webhook URL, signing secret, API token, agent key, and display name all live in environment variables, and `app_settings` is one global row. There is no self-serve way to register an agent, issue/rotate its token, set its webhook from the UI, or see real connection health. To make this a product others can plug their own Arc into, the connection must become a **first-class, operator-managed, set-up-able** thing — without yet building multi-tenancy.

## Goal & non-goals

**Goal:** Promote the env-var "agent port" into a first-class **Agent Connection**: operator-managed config + app-issued tokens (issue/rotate/revoke) + webhook URL/secret + connectivity test + live health, with a **published v1 contract** others build their Arc against. Designed so multi-tenancy or self-host can layer on later with no rework.
...
**Non-goals (deferred, explicitly out of scope):** multi-tenancy (workspaces, auth, RLS isolation); billing; packaging a runnable reference Arc agent (lives in the separate `marketing-classifier-agent` repo). The only seam left for these is the `workspace_id` column.

## Architecture

### The Agent Port module — `src/lib/agent/`

Consolidate connection resolution (today scattered across `src/lib/arc-chat/agent-config.ts` and `notify.ts` reading `process.env`/`getAppSettings` directly) into one module. Every caller uses one resolver:
...
| `display_name` | text null | replaces `ARC_DISPLAY_NAME` |
| `agent_key` | text null | attach key; replaces `ARC_AGENT_KEY` |
| `webhook_url` | text null | agent wake endpoint; `ARC_RUNNER_URL` override target |
...
1. If env `ARC_AGENT_API_TOKEN` is set and the `Authorization: Bearer` matches it → ok (back-compat, unchanged).
...
- **Inbound (agent → app):** token table (prefix · label · last used · revoke); "Generate token" reveals plaintext once into a copy field; a read-only reference panel listing the endpoints the agent calls + a copy-paste `curl` for `/api/v1/arc/ping`.
...
Versioned `v1` doc, the artifact others build their Arc against:
- **Wake (app → agent):** `POST {webhookUrl}` body `{ type: "arc_chat_message", ...ArcNotifyPayload }`; header `X-Webhook-Signature` = HMAC-SHA256 hex of the raw body keyed by the signing secret; 2xx ack; idle = no calls.
- **Auth (agent → app):** `Authorization: Bearer <token>` against `/api/v1/arc/*`.
...
- **Reply contract:** how Arc posts his reply/steps/actions back.
...
- **Route test:** `checkAgentBearer` accepts the env token **and** a DB token, rejects a revoked token, returns `503` when nothing is configured. Existing Arc route tests (`ping`, `tasks`, `messages`, `health`, `drafts`) must stay green after the swap to `checkAgentBearer`.
...
- **Modify:** `src/lib/auth/api-token.ts` (add `checkAgentBearer`); all `src/app/api/v1/arc/**/route.ts` (swap to `checkAgentBearer`); `src/lib/arc-chat/{agent-config,notify}.ts` (source from resolver); `MarkConnection` + `getMarkAgentStatus()`; settings nav.
