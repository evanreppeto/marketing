# Mark Chat Realtime Push — Design
...
The Hermes agent (Mark) on the Mac mini is connected and answering operator chat
messages — but only through the slow safety-net poller. The fast "instant wake"
path is broken: `MARK_RUNNER_URL` pointed at an ephemeral `trycloudflare` quick
...
- `agents`: a `mark` agent row exists, `status: active`, outbound actions blocked
  (`send_email`, `publish_social`, `launch_ads`, …). Safety boundary intact.
- `agent_api_tokens`: none issued — auth relies solely on the static
  `HERMES_AGENT_API_TOKEN` env var (set on Vercel; empty in local `.env.local`).
- Instant-wake push: **broken** (tunnel host does not resolve).
- Only seed/demo rows in `mark_messages` / `agent_tasks` — no recent real
  round-trips.
...
`task_type = mark_chat_message`. The app already inserts that row the instant an
operator sends a message (`enqueueMarkChatTask`). Supabase pushes the event down
the open socket in sub-second time; the Mac mini claims the task, runs Mark, and
...
1. **App (Vercel) — no delivery-logic change.** `enqueueMarkChatTask`
   (`src/lib/mark-chat/enqueue.ts`) already inserts the `agent_tasks` row with
   `task_type: "mark_chat_message"`, `status: "queued"`, and message context in
   `objective` + `metadata`.
...
3. **`mark-runner/realtime_subscriber.py`** (new) — the low-latency receiver:
   ...
   - Call `generate_reply(message)` then `deliver(agentTaskId, text, status)`,
     imported from a **configurable module** (env `MARK_WORKER_MODULE`, default
     `poller`) — matching how `webhook.py` imports today. Whatever Mark's real
...
4. **`mark-runner/com.bsr.mark-realtime.plist`** (new) — a `launchd` job so the
   subscriber auto-starts on boot and restarts on crash (`KeepAlive`).

5. **Docs/config** — update `mark-runner/.env.example` (new `SUPABASE_URL`,
   `SUPABASE_SERVICE_ROLE_KEY`, `MARK_WORKER_MODULE`), `mark-runner/README.md`
   (run the subscriber; phasing note), and `mark-runner/requirements.txt`
   (add `realtime`). Poller and webhook stay stdlib-only; the subscriber is the
   one component that takes the dependency.

6. **Diagnostic** — productionize the throwaway `scripts/diagnose-hermes-connection.mjs`
   (read-only live connection report) and add a `pnpm` alias
   (`diagnose:hermes`) so connection state is checkable on demand.
...
operator send
  ...
  → generate_reply(message)                    [Mark's turn]
  → POST /api/v1/hermes/messages               [already built; settles task]
...
- **Reply idempotency:** `POST /api/v1/hermes/messages` 404s if the pending
...
- Future least-privilege option (out of scope now): swap the service-role key for
  an anon key + a narrow `SELECT` RLS policy on `task_type='mark_chat_message'`
...
- **Live end-to-end:** insert a real `queued` `mark_chat_message` task; confirm
...
- **You do:** put `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (and, if needed,
  `MARK_WORKER_MODULE`) in the Mac mini's `mark-runner/.env`; run the subscriber
  / load the launchd job; confirm Vercel has `HERMES_AGENT_API_TOKEN`.
