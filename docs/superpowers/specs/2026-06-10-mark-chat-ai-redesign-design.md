# Mark Chat — AI-Native Visual Redesign

**Date:** 2026-06-10
**Status:** Approved direction, pending spec review
**Surface:** `src/app/mark/` (operator ↔ Mark conversational workbench)

## Goal

A bold visual overhaul of the Mark chat surface that incorporates modern AI-chat
...
gold, serif for Mark's voice, no emoji / no glow / no gradient / no purple "AI"
aesthetic). All existing wiring is preserved; this is a presentation + interaction
upgrade, not a backend change.

## Non-Goals

- No changes to server actions, persistence (`src/lib/mark-chat/`), polling
...
4. Preserve serif for Mark's voice, mono for identifiers/keys, display for labels.
...
### 2. Mark's responses — `message-list.tsx`

Keep the flat full-width layout (avatar + name/time line + markdown body). Modernize:

- **Reasoning trace:** restyle `StepTrace` ("What Mark did") and the live
...
what Mark does, within Signal tokens. No new data.
...
A CSS-only reveal on Mark's incoming partial `body` (the worker already streams
...
- **Constraint (honest):** text arrives in poll-sized chunks, not per-token. The
  ...
  existing `msg-rise` / `mark-skel` / `mark-ring`), gated by `prefers-reduced-motion`.
...
Single branch (current `feat/mark-kanban-board` or a fresh `feat/mark-ai-redesign`),
