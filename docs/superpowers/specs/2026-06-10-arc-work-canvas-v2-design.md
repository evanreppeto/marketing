...
Date: 2026-06-10
Branch: `feat/arc-kanban-board`
Status: Approved design → ready for implementation plan

## Problem

The Arc Work Canvas (`src/app/arc/_components/work-canvas.tsx`) renders an agent
...
### Domain — `src/domain/arc-canvas.ts` (pure, unit-tested)
...
### Actions — `src/app/arc/actions.ts`
...
- Domain unit tests in `src/domain/__tests__/arc-canvas.test.ts`:
...
per-project asset library), live collaborative editing, re-prompting Arc from an edit, and
...
- Linear history on `feat/arc-kanban-board`.
