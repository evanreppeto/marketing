# Arc Chat — Foundation Pass (Schema Fix + Premium Polish)

**Date:** 2026-06-09
**Status:** Approved (design phase)
**Scope owner:** Arc chat surface (`src/app/arc/**`, `src/lib/arc-chat/**`, `supabase/migrations/**`)
...
supersedes the scope of `2026-06-09-arc-chat-polish-design.md` by adding the missing
...
Turn `/arc` into a professional, complete marketing-assistant workspace the team uses
daily to talk to Arc, make campaigns, find leads, and reference memories. Every button
...
- No external **connection plumbing** — Arc (the external Arc agent) already holds the
  Gmail/Drive/Linear/etc. plugins; the app only surfaces them later (header slot left).
- No change to the Arc webhook contract (`/api/v1/arc/messages/**`) or the
  Mac-side `arc-chat-responder` skill.
- No live AI in the app; outbound stays locked. Arc replies via the existing enqueue →
  pending bubble → poll → complete flow.

## The bug being fixed (load-bearing)

The phase-1 projects code shipped in `src/lib/arc-chat/persistence.ts` —
`CONVERSATION_COLUMNS` selects `project_id`, and `createProject`/`assignConversationToProject`
read/write a `arc_projects` table — but **no migration ever created them**. The only shipped
...
Repo convention: `src/lib/arc-chat/` (I/O, persistence) → `src/app/arc/` (server page +
colocated `_components/`). Server actions in `src/app/arc/actions.ts` stay gated by
`requireOperator()` + `isSupabaseAdminConfigured()` and `revalidatePath("/arc")`.

### Migration — one new additive file

`<ts>_arc_projects_and_pins.sql`:

- `create table public.arc_projects` — `id uuid pk default gen_random_uuid()`,
  ...
  trigger (mirrors `arc_conversations`).
- `alter table public.arc_conversations add column project_id uuid references
  public.arc_projects(id) on delete set null` — deleting a project orphans its chats
  (they fall back to "Chats"), never deletes them.
- `alter table public.arc_conversations add column pinned_at timestamptz` — the pin
  feature, folded in so this is one migration not two.
- `create index arc_conversations_pin_idx on public.arc_conversations(operator,
  pinned_at desc nulls last, last_message_at desc)` — keeps the list query cheap.
- `create index arc_projects_operator_idx on public.arc_projects(operator)`.

Additive only; never edits a shipped migration.

### Data layer (`src/lib/arc-chat/persistence.ts`)
...
- New helpers (mirror existing ones, guarded by `assertOk`):
  ...
  - `deleteConversation(id)` — hard-delete; `arc_messages` cascade via existing FK.
  - `cancelPendingMarkMessage(conversationId)` — deletes the latest `pending` arc message
...
- `deleteThreadForm(formData)` — delete; if it was active the client navigates to `/arc`.
...
## Component design (`src/app/arc/_components/`)

### New / extracted units

- `use-thread-poll.ts` (hook) — extracts the inline pending-reply polling loop from
  `arc-chat.tsx` (`sameMessages` + interval + ~10min safety cap). Makes the shell thin and
...
- Keeps the Operations link; gains a `⋯` (same `ThreadMenu`) for the active thread, and a
  reserved (empty for now) slot where the future "what Arc can reach" connections indicator
  will live.
- Polling moves into `useThreadPoll`.

### `message-list.tsx`

- **Hover toolbar** per row (hover/focus, keyboard reachable): Arc replies get **Copy**
  (clipboard + 1.5s "Copied" confirm); failed replies get **Retry** (re-submits the last
  operator message via a composer callback).
- **Richer thinking state** (pending arc message):
  - Avatar breathes/glows (CSS, transform/opacity only).
  - `steps.length === 0`: shimmer "Arc is thinking…" line (reduced-motion → static).
...
- **Stop**: optimistically removes the pending bubble, stops the poll, calls
  ...
  acceptable.
- **References cluster** (zero-backend slice of "action cards"): when a Arc reply carries
...
`arc-chat.tsx` becomes a thin shell (no inline polling); row logic lives in `ThreadMenu`;
...
- **Connections indicator** — read-only surface of the plugins Arc already holds
  (Gmail/Drive/Linear/Mailchimp/Stripe). (Header slot reserved.)
- Full structured **action-result cards** (needs a Arc-contract change).
- Message **feedback** (👍/👎) to tune Arc.
