...
describe("createGrowthEngineClient", () => {
  it("pings the hosted workspace with the bearer token", async () => {
    ...
    expect(calls).toEqual([
      {
        url: "https://acme.growthengine.com/api/v1/hermes/ping",
  ...
  it("pulls messages and posts replies through the app contract", async () => {
    ...
    expect(calls[0].url).toBe("https://acme.growthengine.com/api/v1/hermes/messages?limit=5");
    expect(calls[0].init.method).toBe("GET");
    expect(calls[1].url).toBe("https://acme.growthengine.com/api/v1/hermes/messages");
...
describe("connector setup helpers", () => {
  it("renders the Hermes-side env template", () => {
    expect(
      createEnvTemplate({
        baseUrl: "https://acme.growthengine.com/",
        token: "sk_live_test",
  ...
  it("verifies app wake webhook HMAC signatures", () => {
    const body = JSON.stringify({ type: "mark_chat_message", agentTaskId: "task-1" });
