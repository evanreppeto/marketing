...
export function parseInitArgs(args) {
  const [command] = args;
  if (command !== "init") {
    return { ok: false, message: "Usage: growth-arc init --app <url> --token <token> [--secret <secret>] [--env <file>]" };
  }
...
export async function runInitCommand(args, { writeFile, stdout = () => {} }) {
  ...
  stdout("Add these values to the Arc agent runtime, then call client.ping() to verify the connection.");
