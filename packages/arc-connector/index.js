...
export function createGrowthEngineClient({ baseUrl, token, fetch: fetchImpl = globalThis.fetch }) {
  ...
  return {
    baseUrl: root,
    ping: () => request("/api/v1/arc/ping"),
    listMessages: ({ limit = 20 } = {}) => request(`/api/v1/arc/messages?limit=${encodeURIComponent(String(limit))}`),
    reply: ({ agentTaskId, body, status = "complete", metadata = {} }) =>
      request("/api/v1/arc/messages", {
        method: "POST",
        body: { agentTaskId, body, status, metadata },
      }),
    reportSteps: ({ agentTaskId, steps, body }) =>
      request(`/api/v1/arc/messages/${encodeURIComponent(agentTaskId)}/steps`, {
        method: "POST",
...
export function createEnvTemplate({ baseUrl, token, webhookSecret = "" }) {
  ...
  return [
    ...
    `ARC_WEBHOOK_SECRET=${String(webhookSecret ?? "").trim()}`,
    "",
  ].join("\n");
}
