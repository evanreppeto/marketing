...
- Product posture: this app is primarily a backend/control plane for the **Arc** agent (surfaced in the UI as **Arc**). Build durable APIs, records, queues, approvals, logs, and state transitions first. UI pages are detailed operator views for humans and agent debugging, not the main source of product value.
...
- Live API surfaces (each carries its own auth — see Operator Auth & API Tokens):
  ...
  - `POST /api/v1/arc/runs` — bearer-gated (`ARC_AGENT_API_TOKEN`); runs `runHermesPartnerCampaign` from `@/lib/arc/orchestrator`. Returns `503 not_configured` if Supabase admin isn't set up. `POST /api/v1/arc/ping` is the bearer-gated health/connectivity check.
