...
- `src/app/_components/app-shell.tsx` reads `navItems` from `src/app/_data/growth-engine.ts` (current top-level nav: Today `/`, Activity `/approvals`, CRM `/crm`, Personas `/personas`, Arc `/arc`, Settings `/settings`). Adding a top-level page = add an entry to `navItems`.
