 ...
 * card soup, gold used sparingly by the caller's headline + primary action.
 */
export function AuthShell({
  ...
  return (
    <main className="chicago-dark grid min-h-[100dvh] bg-[var(--canvas)] text-[var(--text-primary)] md:grid-cols-[1.05fr_0.95fr]">
      <section className="relative hidden flex-col justify-between border-r border-[var(--border-hairline)] px-10 py-12 md:flex lg:px-14">
        <Wordmark />
        ...
        {meta && meta.length ? (
          <div className="flex flex-wrap items-center gap-x-3.5 gap-y-2 text-xs text-[var(--text-muted)]">
            {meta.map((item, index) => (
      ...
      <section className="flex min-h-[100dvh] flex-col justify-center px-5 py-10 sm:px-8">
        <div className={`w-full ${formMaxWidth} mx-auto`}>
