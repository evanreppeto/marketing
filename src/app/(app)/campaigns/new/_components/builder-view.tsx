...
function strip(state: string, name: string, meta?: string) {
  if (state === "ok") return `<div class="papprove ok"><span class="picon">${CK}</span><div class="pinfo"><div class="pt">${name} — approved</div><div class="pm">Approved by Riley · 2h ago · ready to launch</div></div><div class="pa-acts"><span class="undo">Undo</span></div></div>`;
  return `<div class="papprove pend"><span class="picon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="9"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg></span><div class="pinfo"><div class="pt">${name} — needs your review</div><div class="pm">${meta}</div></div><div class="pa-acts"><span class="sbtn">Request revision</span><span class="sbtn go">Approve this piece</span></div></div>`;
}
...
function acard(a: (typeof ASSET)[number]) {
  ...
  return `<div class="acard"><div class="thmb">${a[1]}<span class="ttl">${ic}${a[0] === "video" ? "Video" : "Image"}</span><span class="fmt">${a[4]}</span>${play}<span class="prov ${a[2]}">${a[3]}</span><div class="ahover"><span class="hb go">Approve</span><span class="hb">Replace</span></div></div><div class="ameta"><span class="asrc">${a[5]}</span><span class="astat ${sc}">${a[7]}</span></div></div>`;
}
...
function tabHtml(t: string, brand: string): string {
  switch (t) {
    ...
    case "media":
      return `<div class="mtoolbar"><span class="mc">6 assets · 1 needs review</span><span class="fchip on">All</span><span class="fchip">Images</span><span class="fchip">Video</span><span class="fchip">Real</span><span class="fchip">AI</span><div class="acts"><span class="mbtn gold"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8z"/></svg>Generate with Arc</span><span class="mbtn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 7h6l2 2h8v10H4z"/></svg>Add from Library</span></div></div><div class="mgrid">${ASSET.map(acard).join("")}</div>`;
    default:
      return "";
  }
}
