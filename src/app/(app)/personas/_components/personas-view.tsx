...
function Radar({ sig }: { sig: { engagement: number; fit: number; intent: number } }) {
  ...
  return (
    <svg viewBox="0 0 170 138" width="172" height="140" aria-hidden="true">
      ...
      {ax.map((a) => { const lp = pt(a[1], R + 13); return <text key={a[0]} x={lp[0].toFixed(1)} y={(lp[1] + 3).toFixed(1)} textAnchor="middle" fontSize="9.5" fontWeight="600" fill="#b6b1a6">{a[2]}</text>; })}
    </svg>
  );
}
