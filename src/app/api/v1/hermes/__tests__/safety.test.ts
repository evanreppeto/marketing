...
/**
 * Structural safety guarantee for the Mark Operations API: none of the agent
 * route handlers may import the human decision / launch / dispatch paths. That
 * omission — not a runtime check — is what proves Mark can never approve,
...
describe("Mark Operations API outbound safety", () => {
  const files = AGENT_DIRS.flatMap((sub) => {
    try {
      return collectSourceFiles(join(HERMES_DIR, sub));
    } catch {
      return [];
    }
  });
