...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        message: "Supabase admin env vars are required before Hermes can persist work.",
      },
      { status: 503 },
    );
  }
  ...
  try {
    const result = await runHermesPartnerCampaign(payload);
    ...
    return NextResponse.json(
      {
        ok: false,
        status: "failed",
        message: error instanceof Error ? error.message : "Hermes run failed.",
      },
      { status: 502 },
    );
  }
