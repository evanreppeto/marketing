...
function healthRequest(authorization?: string) {
  return new Request("http://localhost/api/v1/hermes/health", {
    headers: authorization ? { authorization } : {},
  });
}

describe("GET /api/v1/hermes/health", () => {
  ...
  it("returns 200 with the service name and DB readiness", async () => {
    ...
    expect(body).toMatchObject({ ok: true, status: "ok", service: "bsr-marketing-hermes" });
