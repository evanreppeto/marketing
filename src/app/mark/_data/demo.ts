...
/**
 * Preview-mode sample data so the full Mark chat UI renders without Supabase or
...
function conv(id: string, title: string, agoMs: number, extra?: Partial<MarkConversation>): MarkConversation {
  return {
    id,
    operator: "demo",
    title,
...
const CONVERSATIONS: MarkConversation[] = [
...
// The campaign package Mark drafted — six assets across channels, mixed provenance.
...
const CAMPAIGN_ASSETS: MarkActionCard[] = [
  {
    kind: "draft",
...
/** Full MarkChat prop bag for preview mode. */
...
/** A canned Mark reply for preview-mode sends (no backend). */
export function demoReply(toBody: string): MarkMessage {
  return {
