...
import type { MarkActionCard, MarkAssetStatus, MarkMedia, MarkMediaSource } from "@/domain";
...
export function riskReasons(card: MarkActionCard): string[] {
  const reasons = [
...
const SOURCE_LABEL: Record<MarkMediaSource, string> = {
  bsr_real: "Real BSR",
  ai_generated: "AI",
  composite: "Composite",
  stock: "Stock",
...
const STATUS_LABEL: Record<MarkAssetStatus, string> = {
  draft: "Draft",
...
export function SourceBadge({ source }: { source: MarkMediaSource }) {
  const tone =
    source === "bsr_real"
...
export function StatusPill({ status }: { status: MarkAssetStatus }) {
  const tone =
    status === "approved"
      ? "text-[var(--ok-text)] bg-[var(--ok-soft)]"
      : status === "rejected"
...
export function MediaProvenance({ media, className }: { media: MarkMedia; className?: string }) {
