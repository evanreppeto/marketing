...
export async function generateMetadata({ params }: { params: Promise<{ recordId: string }> }): Promise<Metadata> {
  try {
    ...
    const data = await getCrmRecordData("properties", recordId);
    return { title: (data.status === "live" ? data.name?.trim() : null) || "Property" };
  } catch {
...
export default async function Page({ params, searchParams }: PageProps) {
  ...
  return <CrmRecordPage action={getValue(query.action)} tab={getValue(query.tab)} objectKey="properties" recordId={recordId} />;
}
