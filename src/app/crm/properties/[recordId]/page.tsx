...
export function generateStaticParams() {
  return getCrmRecordParams("properties");
}

export default async function PropertyRecordPage({ params, searchParams }: DetailPageProps) {
  const { recordId } = await params;
  const query: { action?: string | string[] } = searchParams ? await searchParams : {};

  return <CrmRecordPage action={getAction(query.action)} objectKey="properties" recordId={recordId} />;
}
