...
export function BrandSourceUpload({ placement = "inline" }: { placement?: "hero" | "inline" }) {
  ...
  return (
    <form
      action={action}
      className={isHero ? "bg-[var(--surface-panel)] px-5 py-5" : "border-b border-[var(--border-hairline)] bg-[var(--surface-inset)] px-5 py-5"}
      encType="multipart/form-data"
    >
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <label className="flex min-h-24 min-w-0 cursor-pointer items-center gap-4 rounded-md border border-dashed border-[var(--accent-border-strong)] bg-[var(--surface-soft)] px-4 py-4 text-sm text-[var(--text-secondary)] transition hover:border-[var(--accent)] hover:bg-[var(--surface-inset)]">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-[var(--border-hairline)] bg-[var(--surface-inset)] text-[var(--accent)]">
