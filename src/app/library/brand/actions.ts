...
export async function applyBrandDesignAction(
  ...
  if (logoUrl && (!current.logoUrl || overwrite)) {
    const stored = await storeBrandImageFromUrl({ orgId, url: logoUrl, role: "logo", sourceUrl, uploadedBy });
  ...
  if (faviconUrl && (!current.faviconUrl || overwrite)) {
    const stored = await storeBrandImageFromUrl({ orgId, url: faviconUrl, role: "favicon", sourceUrl, uploadedBy });
