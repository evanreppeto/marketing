...
export async function isArcRunnerConfigured(): Promise<boolean> {
  const connection = await resolveAgentConnection();
  return Boolean(connection.webhookUrl && connection.enabled);
}

export async function getArcDisplayName(): Promise<string> {
  const connection = await resolveAgentConnection();
...
export function getAgentDisplayName(override: string | null | undefined): string {
  return override?.trim() || process.env.ARC_DISPLAY_NAME?.trim() || "Arc";
}
 ...
 * /api/v1/arc call stamps agent_connections.last_seen_at (the poller's poll, a
