export type HermesRiskLevel = "low" | "medium" | "high" | "blocked";

export type HermesGuardrailResult = {
...
export function checkHermesGeneratedCopy(input: {
  draftOutput: string;
  lossSignals?: string[];
  restorationFocus?: string;
}): HermesGuardrailResult {
  ...
  if (blockedPhrases.length > 0 || offScopeLoss) {
    return {
      ...
      complianceNotes: "Blocked by Hermes guardrails. Rewrite before owner approval.",
      flags: [...flags],
      blockedPhrases,
    };
  }
