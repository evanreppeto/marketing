...
const sourceSystem = "arc_demo_workflow";

export async function runArcDemoWorkflow(client?: SupabaseClient): Promise<ArcDemoWorkflowResult> {
  const supabase = client ?? getSupabaseAdminClient();
  const runId = new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14);
  const agentId = await upsertArcDemoAgent(supabase);

  await ensureGuardrails(supabase);

  const companyId = await insertOne(supabase, "companies", {
    name: `Arc Plumbing Partner ${runId}`,
    persona,
    status: "active",
  ...
  const leadId = await insertOne(supabase, "leads", {
    ...
    source: "arc_demo",
    external_lead_id: `arc-demo-plumbing-${runId}`,
    loss_summary:
      "Plumbing company in priority Chicago ZIPs. Arc recommends partner outreach for water-loss referral handoff.",
  ...
  const campaignId = await insertOne(supabase, "campaigns", {
    ...
    external_campaign_id: `arc-demo-campaign-${runId}`,
    launch_locked: true,
  ...
  await insertOne(supabase, "persona_knowledge_entries", {
    ...
    priority: 88,
    status: "active",
    source_reference: `arc_demo_workflow:${runId}`,
    metadata: {
      demo_workflow: true,
      run_id: runId,
    },
  ...
  const approvalItemId = await insertOne(supabase, "approval_items", {
    ...
    requested_by: "Arc Demo Workflow",
    risk_level: "medium",
    compliance_notes:
  ...
  await insertOne(supabase, "agent_run_logs", {
    ...
    model_provider: "seed",
    model_name: "arc-demo-workflow",
    input_token_count: 0,
    output_token_count: 0,
    cost_estimate_cents: 0,
    reasoning_summary:
      "Created a linked company, contact, lead, persona snapshot, campaign, asset, approval item, task input, output, and audit log for the Arc approval workflow.",
    started_at: new Date().toISOString(),
    completed_at: new Date().toISOString(),
    metadata: {
      demo_workflow: true,
...
async function upsertArcDemoAgent(supabase: SupabaseClient) {
    ...
    .upsert(
      {
        ...
        description: "Demo orchestrator that creates reviewable lead and campaign work before live Arc is connected.",
        status: "ready",
