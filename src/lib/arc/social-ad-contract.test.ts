...
describe("parseArcSocialAdRequest", () => {
  it("accepts a request with multiple image assets (base64)", () => {
    const req = parseArcSocialAdRequest({
      ...base,
      assets: [
  ...
  it("rejects an empty assets array", () => {
    expect(() => parseArcSocialAdRequest({ ...base, assets: [] })).toThrow();
  });

  it("rejects an asset missing imageBase64", () => {
    expect(() => parseArcSocialAdRequest({ ...base, assets: [{ format: "feed_1080x1080" }] })).toThrow();
  });

  it("rejects an invalid persona", () => {
    expect(() =>
      parseArcSocialAdRequest({ ...base, persona: "unassigned_persona", assets: [{ imageBase64: PNG_B64 }] }),
    ).toThrow();
  });
});
