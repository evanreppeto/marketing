#!/usr/bin/env python3
"""
MCP server exposing the BSR "Mark Operations API" as tools for the Hermes/Mark
agent, so Mark calls native tools instead of hand-crafting HTTP requests.

Every tool is a thin wrapper over a bearer-gated endpoint on the deployed app
(GET/POST {BASE_URL}/api/v1/hermes/*). The app enforces all auth and safety, so
this layer adds none of its own — and, by construction, exposes NO tool that
approves, rejects, launches, sends, publishes, or dispatches. Mark can read,
...
Config via env (see .env.example):
    ...
    HERMES_AGENT_API_TOKEN  — same shared secret as the app.
...
mcp = FastMCP("bsr-marketing-hermes")


def _request(method: str, path: str, payload: dict | None = None, params: dict | None = None) -> dict:
    """Call the Mark Operations API and return the parsed JSON body.
...
@mcp.tool()
def health() -> dict:
    """Check the Mark Operations API is reachable and authorized."""
    return _request("GET", "/api/v1/hermes/health")
...
def list_tasks(status: str | None = None, assignee: str | None = None, limit: int | None = None) -> dict:
    ...
    return _request("GET", "/api/v1/hermes/tasks", params={"status": status, "assignee": assignee, "limit": limit})


@mcp.tool()
def get_task(task_id: str) -> dict:
    """Get full detail for one task (context, logs, next_allowed_actions)."""
    return _request("GET", f"/api/v1/hermes/tasks/{task_id}")


@mcp.tool()
def claim_task(task_id: str) -> dict:
    """Claim a queued task (queued -> running)."""
    return _request("POST", f"/api/v1/hermes/tasks/{task_id}/claim")
...
def log_task(
    ...
    return _request(
        "POST",
        f"/api/v1/hermes/tasks/{task_id}/log",
        payload={
            "message": message,
...
def complete_task(task_id: str, summary: str | None = None, outputs: dict | None = None, metadata: dict | None = None) -> dict:
    ...
    return _request(
        "POST",
        f"/api/v1/hermes/tasks/{task_id}/complete",
...
def block_task(task_id: str, reason: str, needs: dict | None = None, metadata: dict | None = None) -> dict:
    """Mark a task blocked with a reason (use when you need a human)."""
    return _request(
        "POST",
        f"/api/v1/hermes/tasks/{task_id}/block",
...
def list_approvals(status: str | None = None, limit: int | None = None) -> dict:
    ...
    return _request("GET", "/api/v1/hermes/approvals", params={"status": status, "limit": limit})


@mcp.tool()
def get_approval(approval_id: str) -> dict:
    """Get one approval item, including Mark's prior recommendations."""
    return _request("GET", f"/api/v1/hermes/approvals/{approval_id}")


@mcp.tool()
def list_approval_recommendations(approval_id: str) -> dict:
    """Read back the recommendations left on an approval item."""
    return _request("GET", f"/api/v1/hermes/approvals/{approval_id}/recommendations")
...
def add_approval_recommendation(
    ...
    return _request(
        "POST",
        f"/api/v1/hermes/approvals/{approval_id}/recommendation",
        payload={
...
def list_campaigns(status: str | None = None, needs_review: bool | None = None, limit: int | None = None) -> dict:
    ...
    return _request("GET", "/api/v1/hermes/campaigns", params=params)


@mcp.tool()
def get_campaign(campaign_id: str) -> dict:
    """Get full campaign workspace detail."""
    return _request("GET", f"/api/v1/hermes/campaigns/{campaign_id}")
